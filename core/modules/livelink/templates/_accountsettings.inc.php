...
<p><?= __('Link your external services (such as GitHub, GitLab, etc) from this page to enable linking projects via %tbg_live_link', ['%tbg_live_link' => link_tag('https://thebuggenie.com/features/livelink', fa_image_tag('leaf tbg-livelink') . 'TBG Live Link')]); ?></p>
    ...
    <ul id="livelink-connector-accounts" class="livelink_connector_accounts">
        <?php foreach ($module->getConnectorModules() as $connector_key => $connector_provider): ?>
            <li id="livelink-<?= $connector_key; ?>-configuration" class="<?= ($connector_provider->getConnector()->isConfigured()) ? 'connected' : ''; ?>">
                <span class="description"><?= fa_image_tag($connector_provider->getConnector()->getLogo()) . $connector_provider->getConnector()->getName(); ?><span class="not-connected"><?= fa_image_tag('square-o') . __('Not connected'); ?></span><span class="connected-ok"><?= fa_image_tag('check-square-o') . __('Connected'); ?></span></span>
