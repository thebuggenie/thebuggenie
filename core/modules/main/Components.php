<?php
     ...
     *
     * @property \thebuggenie\core\entities\Issue $issue The issue
     *
     */
    class Components extends framework\ActionComponent
    {
        ...
        public function componentUserdropdown()
        {
            ...
            try
            {
                if (!$this->user instanceof \thebuggenie\core\entities\User)
                {
                    framework\Logging::log('loading user object in dropdown');
                    if (is_numeric($this->user))
                    {
                        $this->user = Users::getTable()->getByUserId($this->user);
                    }
                    else
                    {
                        $this->user = Users::getTable()->getByUsername($this->user);
                    }
        ...
        public function componentClientusers()
        {
            try
            {
                if (!$this->client instanceof \thebuggenie\core\entities\Client)
                {
                    framework\Logging::log('loading user object in dropdown');
                    $this->client = \thebuggenie\core\entities\Client::getB2DBTable()->selectById($this->client);
                    framework\Logging::log('done (loading user object in dropdown)');
                }
        ...
        public function componentArchivedProjects()
        {
            ...
            {
                $this->projects = \thebuggenie\core\entities\Project::getAllRootProjects(true);
                $this->project_count = count($this->projects);
            }
            elseif ($this->target == 'team')
            {
                $this->team = \thebuggenie\core\entities\Team::getB2DBTable()->selectById($this->id);
                $projects = array();
                foreach (\thebuggenie\core\entities\Project::getAllByOwner($this->team) as $project)
                {
                    $projects[$project->getID()] = $project;
                }
                foreach (\thebuggenie\core\entities\Project::getAllByLeader($this->team) as $project)
                {
                    $projects[$project->getID()] = $project;
                }
                foreach (\thebuggenie\core\entities\Project::getAllByQaResponsible($this->team) as $project)
                {
                    $projects[$project->getID()] = $project;
                }
            ...
            {
                $this->client = \thebuggenie\core\entities\Client::getB2DBTable()->selectById($this->id);
                $projects = \thebuggenie\core\entities\Project::getAllByClientID($this->client->getID());
            ...
            {
                $this->parent = \thebuggenie\core\entities\Project::getB2DBTable()->selectById($this->id);
        ...
        public function componentTeamdropdown()
        {
            ...
            try
            {
                ...
                if (!$this->team instanceof \thebuggenie\core\entities\Team)
                {
                    framework\Logging::log('loading team object in dropdown');
                    $this->team = \thebuggenie\core\entities\Team::getB2DBTable()->selectById($this->team);
        ...
        protected function setupVariables()
        {
            ...
            if ($this->issue instanceof \thebuggenie\core\entities\Issue)
            {
                $this->project = $this->issue->getProject();
                $this->statuses = ($this->project->isFreelancingAllowed()) ? \thebuggenie\core\entities\Status::getAll() : $this->issue->getAvailableStatuses();
                $this->issuetypes = $this->project->getIssuetypeScheme()->getIssuetypes();
                $fields_list = array();
                $fields_list['category'] = array('title' => $i18n->__('Category'), 'choices' => array(), 'visible' => $this->issue->isCategoryVisible(), 'changed' => $this->issue->isCategoryChanged(), 'merged' => $this->issue->isCategoryMerged(), 'name' => (($this->issue->getCategory() instanceof \thebuggenie\core\entities\Category) ? $this->issue->getCategory()->getName() : ''), 'name_visible' => (bool) ($this->issue->getCategory() instanceof \thebuggenie\core\entities\Category), 'noname_visible' => (bool) (!$this->issue->getCategory() instanceof \thebuggenie\core\entities\Category), 'icon' => false, 'change_tip' => $i18n->__('Click to change category'), 'change_header' => $i18n->__('Change category'), 'clear' => $i18n->__('Clear the category'), 'select' => $i18n->__('%clear_the_category or click to select a new category', array('%clear_the_category' => '')));
                if ($this->issue->isUpdateable() && $this->issue->canEditCategory())
                    $fields_list['category']['choices'] = \thebuggenie\core\entities\Category::getAll();
                $fields_list['resolution'] = array('title' => $i18n->__('Resolution'), 'choices' => array(), 'visible' => $this->issue->isResolutionVisible(), 'changed' => $this->issue->isResolutionChanged(), 'merged' => $this->issue->isResolutionMerged(), 'name' => (($this->issue->getResolution() instanceof \thebuggenie\core\entities\Resolution) ? $this->issue->getResolution()->getName() : ''), 'name_visible' => (bool) ($this->issue->getResolution() instanceof \thebuggenie\core\entities\Resolution), 'noname_visible' => (bool) (!$this->issue->getResolution() instanceof \thebuggenie\core\entities\Resolution), 'icon' => false, 'change_tip' => $i18n->__('Click to change resolution'), 'change_header' => $i18n->__('Change resolution'), 'clear' => $i18n->__('Clear the resolution'), 'select' => $i18n->__('%clear_the_resolution or click to select a new resolution', array('%clear_the_resolution' => '')));
                if ($this->issue->isUpdateable() && $this->issue->canEditResolution())
                    $fields_list['resolution']['choices'] = \thebuggenie\core\entities\Resolution::getAll();
                $fields_list['priority'] = array('title' => $i18n->__('Priority'), 'choices' => array(), 'visible' => $this->issue->isPriorityVisible(), 'changed' => $this->issue->isPriorityChanged(), 'merged' => $this->issue->isPriorityMerged(), 'name' => (($this->issue->getPriority() instanceof \thebuggenie\core\entities\Priority) ? $this->issue->getPriority()->getName() : ''), 'name_visible' => (bool) ($this->issue->getPriority() instanceof \thebuggenie\core\entities\Priority), 'noname_visible' => (bool) (!$this->issue->getPriority() instanceof \thebuggenie\core\entities\Priority), 'icon' => false, 'change_tip' => $i18n->__('Click to change priority'), 'change_header' => $i18n->__('Change priority'), 'clear' => $i18n->__('Clear the priority'), 'select' => $i18n->__('%clear_the_priority or click to select a new priority', array('%clear_the_priority' => '')));
                if ($this->issue->isUpdateable() && $this->issue->canEditPriority())
                    $fields_list['priority']['choices'] = \thebuggenie\core\entities\Priority::getAll();
                $fields_list['reproducability'] = array('title' => $i18n->__('Reproducability'), 'choices' => array(), 'visible' => $this->issue->isReproducabilityVisible(), 'changed' => $this->issue->isReproducabilityChanged(), 'merged' => $this->issue->isReproducabilityMerged(), 'name' => (($this->issue->getReproducability() instanceof \thebuggenie\core\entities\Reproducability) ? $this->issue->getReproducability()->getName() : ''), 'name_visible' => (bool) ($this->issue->getReproducability() instanceof \thebuggenie\core\entities\Reproducability), 'noname_visible' => (bool) (!$this->issue->getReproducability() instanceof \thebuggenie\core\entities\Reproducability), 'icon' => false, 'change_tip' => $i18n->__('Click to change reproducability'), 'change_header' => $i18n->__('Change reproducability'), 'clear' => $i18n->__('Clear the reproducability'), 'select' => $i18n->__('%clear_the_reproducability or click to select a new reproducability', array('%clear_the_reproducability' => '')));
                if ($this->issue->isUpdateable() && $this->issue->canEditReproducability())
                    $fields_list['reproducability']['choices'] = \thebuggenie\core\entities\Reproducability::getAll();
                $fields_list['severity'] = array('title' => $i18n->__('Severity'), 'choices' => array(), 'visible' => $this->issue->isSeverityVisible(), 'changed' => $this->issue->isSeverityChanged(), 'merged' => $this->issue->isSeverityMerged(), 'name' => (($this->issue->getSeverity() instanceof \thebuggenie\core\entities\Severity) ? $this->issue->getSeverity()->getName() : ''), 'name_visible' => (bool) ($this->issue->getSeverity() instanceof \thebuggenie\core\entities\Severity), 'noname_visible' => (bool) (!$this->issue->getSeverity() instanceof \thebuggenie\core\entities\Severity), 'icon' => false, 'change_tip' => $i18n->__('Click to change severity'), 'change_header' => $i18n->__('Change severity'), 'clear' => $i18n->__('Clear the severity'), 'select' => $i18n->__('%clear_the_severity or click to select a new severity', array('%clear_the_severity' => '')));
                if ($this->issue->isUpdateable() && $this->issue->canEditSeverity())
                    $fields_list['severity']['choices'] = \thebuggenie\core\entities\Severity::getAll();
                $fields_list['milestone'] = array('title' => $i18n->__('Targetted for'), 'choices' => array(), 'visible' => $this->issue->isMilestoneVisible(), 'changed' => $this->issue->isMilestoneChanged(), 'merged' => $this->issue->isMilestoneMerged(), 'name' => (($this->issue->getMilestone() instanceof \thebuggenie\core\entities\Milestone) ? $this->issue->getMilestone()->getName() : ''), 'name_visible' => (bool) ($this->issue->getMilestone() instanceof \thebuggenie\core\entities\Milestone), 'noname_visible' => (bool) (!$this->issue->getMilestone() instanceof \thebuggenie\core\entities\Milestone), 'icon' => true, 'icon_name' => 'icon_milestones.png', 'change_tip' => $i18n->__('Click to change which milestone this issue is targetted for'), 'change_header' => $i18n->__('Set issue target / milestone'), 'clear' => $i18n->__('Set as not targetted'), 'select' => $i18n->__('%set_as_not_targetted or click to set a new target milestone', array('%set_as_not_targetted' => '')), 'url' => true, 'current_url' => (($this->issue->getMilestone() instanceof \thebuggenie\core\entities\Milestone) ? $this->getRouting()->generate('project_milestone_details', array('project_key' => $this->issue->getProject()->getKey(), 'milestone_id' => $this->issue->getMilestone()->getID())) : ''));
                ...
                foreach (\thebuggenie\core\entities\CustomDatatype::getAll() as $key => $customdatatype)
                {
                    ...
                    if ($customdatatype->getType() == \thebuggenie\core\entities\CustomDatatype::CALCULATED_FIELD)
                    {
                    ...
                    {
                        $customfields_list[$key]['name'] = ($customvalue instanceof \thebuggenie\core\entities\CustomDatatypeOption) ? $customvalue->getName() : '';
                        $customfields_list[$key]['name_visible'] = (bool) ($customvalue instanceof \thebuggenie\core\entities\CustomDatatypeOption);
                        $customfields_list[$key]['noname_visible'] = (bool) (!$customvalue instanceof \thebuggenie\core\entities\CustomDatatypeOption);
            ...
            if (isset($this->transition) && $this->transition->hasAction(\thebuggenie\core\entities\WorkflowTransitionAction::ACTION_ASSIGN_ISSUE))
            {
        ...
        public function componentUploader()
        {
            ...
            {
                case 'issue':
                    $this->issue = \thebuggenie\core\entities\Issue::getB2DBTable()->selectById($this->issue_id);
                    break;
                case 'article':
        ...
        public function componentLogitem()
        {
            if ($this->log_action['target_type'] == 1)
            {
                try
                {
                    $this->issue = \thebuggenie\core\entities\Issue::getB2DBTable()->selectById($this->log_action['target']);
                }
                catch (\Exception $e)
                {
        ...
        public function componentCommentitem()
        {
            if ($this->comment->getTargetType() == 1)
            {
                try
                {
                    $this->issue = \thebuggenie\core\entities\Issue::getB2DBTable()->selectById($this->comment->getTargetID());
                }
                catch (\Exception $e)
                {
        ...
        protected function _setupReportIssueProperties()
        {
            ...
            $this->reproducabilities = \thebuggenie\core\entities\Reproducability::getAll();
            $this->resolutions = \thebuggenie\core\entities\Resolution::getAll();
            $this->statuses = \thebuggenie\core\entities\Status::getAll();
        ...
        {
            $introarticle = Articles::getTable()->getArticleByName(ucfirst(framework\Context::getCurrentProject()->getKey()) . ':ReportIssueIntro');
            $this->introarticle = ($introarticle instanceof Article) ? $introarticle : Articles::getTable()->getArticleByName('ReportIssueIntro');
            $reporthelparticle = Articles::getTable()->getArticleByName(ucfirst(framework\Context::getCurrentProject()->getKey()) . ':ReportIssueHelp');
            ...
            $dummyissue = new \thebuggenie\core\entities\Issue();
        ...
        public function componentIssuePermissions()
        {
            ...
            {
                if ($item['target'] instanceof \thebuggenie\core\entities\User && $item['target']->getID() == $this->getUser()->getID())
                {
                    unset($al_items[$k]);
                }
            }
        ...
        {
            $this->entry = \thebuggenie\core\entities\tables\IssueSpentTimes::getTable()->selectById($this->entry_id);
        }
        ...
        {
            $this->comments = \thebuggenie\core\entities\Comment::getRecentCommentsByAuthor($this->getUser()->getID());
        }
        ...
        public function componentAddDashboardView()
        {
            ...
            $this->savedsearches = \thebuggenie\core\entities\tables\SavedSearches::getTable()->getAllSavedSearchesByUserIDAndPossiblyProjectID(framework\Context::getUser()->getID(), ($this->dashboard->getProject() instanceof \thebuggenie\core\entities\Project) ? $this->dashboard->getProject()->getID() : 0);
        }
