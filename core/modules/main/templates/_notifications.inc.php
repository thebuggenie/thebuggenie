    ...
    <?php else: ?>
        ...
        <li class="<?= ($notification->isRead()) ? 'read' : 'unread'; ?>" id="notification_<?= $notification->getID(); ?>_container" data-notification-id="<?= $notification->getID(); ?>">
            <a href="javascript:void(0);" onclick="TBG.Main.Notifications.toggleRead(<?= $notification->getID(); ?>);" class="notification_status_toggler">
                ...
                switch ($notification->getNotificationType())
                {
                    case \thebuggenie\core\entities\Notification::TYPE_ISSUE_CREATED:
                        ...
                        <h1>
                            ...
                            <time><?= tbg_formatTime($notification->getCreatedAt(), 20); ?></time>
                        ...
                        <div class="notification_content"><?= link_tag(make_url('viewissue', array('project_key' => $notification->getTarget()->getProject()->getKey(), 'issue_no' => $notification->getTarget()->getFormattedIssueNo())), $notification->getTarget()->getFormattedIssueNo(true, true)); ?> - <?= $notification->getTarget()->getTitle(); ?></div>
                        <?php
                        break;
            ...
            ?>
                <script>
                    require(['domReady', 'thebuggenie/tbg'], function (domReady, TBG) {
                        domReady(function () {
                            TBG.Main.Notifications.Web.Send("<?= __('New notification'); ?>", "<?= $notification_text; ?>", '<?= $notification->getID(); ?>', '<?= $notification->getTriggeredByUser()->getAvatarURL(); ?>', function () {
