<?php
    ...
    if ($client instanceof \thebuggenie\core\entities\Client)
    {
        ...
        $tbg_response->addBreadcrumb($client->getName(), make_url('client_dashboard', array('client_id' => $client->getCode())));
    }
    else
    {
...
<div class="client_dashboard">
    <div class="main_area">
        <div class="dashboard_client_info">
            <span class="dashboard_client_header"><?php echo $client->getName(); ?> [<?php echo $client->getCode(); ?>]</span>
            <?php if ($tbg_user->canAccessConfigurationPage(\thebuggenie\core\framework\Settings::CONFIGURATION_SECTION_USERS)): ?>
                <div class="project_header_right button-group">
                    <button class="button button-silver dropper first last" id="client_<?php echo $client->getID(); ?>_more_actions"><?= image_tag('spinning_16.gif', ['id' => 'client_members_' . $client->getID() . '_indicator', 'style' => 'display: none']); ?>&nbsp;<?php echo __('Actions'); ?></button>                    
                    <ul style="margin-top: 28px; font-size: 1.1em;" class="simple_list rounded_box white shadowed popup_box more_actions_dropdown" onclick="$('client_<?php echo $client->getID(); ?>_more_actions').removeClassName('button-pressed');">
                        <li><?php echo javascript_link_tag(fa_image_tag('user-plus').__('Add member(s) to this client'), array('onclick' => '$(\'addmember_client_'.$client->getID().'\').removeClassName(\'popup_box\');$(\'addmember_client_'.$client->getID().'\').toggle(\'block\');')); ?></li>
            ...
            <?php include_component('main/identifiableselector', [
                ...
                'callback'      => "TBG.Config.Client.addMember('".make_url('configure_users_add_client_member', ['client_id' => $client->getID(), 'user_id' => '%identifiable_value'])."', ".$client->getID().", '%identifiable_value');$('addmember_client_{$client->getID()}').toggle();$('addmember_client_{$client->getID()}').addClassName('popup_box');",
                'base_id'       => "addmember_client_{$client->getID()}",
                'include_teams' => false,
                'include_users' => true,
                'allow_clear'   => false,
                'allow_close'   => true,
                'style'         => array('right' => '3em', 'top' => '6.25em', 'display' => 'none'),
                'absolute'      => true
            ]); ?>
            <table>
                <tr>
        ...
        <div class="client_dashboard_projects padded">
