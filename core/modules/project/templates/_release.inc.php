<li class="release_item rounded_box invisible <?php if ($build->isActive()) echo 'active'; ?>" style="line-height: 1.3;">
    <div class="button-group">
        <?php if ($build->isActive()): ?>
            ...
            <?php endif; ?>
            <?php echo link_tag(make_url('project_issues', array('project_key' => TBGContext::getCurrentProject()->getKey(), 'search' => true, 'fs[state]' => array('o' => '=', 'v' => TBGIssue::STATE_OPEN), 'fs[build]' => array('o' => '=', 'v' => $build->getID())))."?sortfields=issues.posted=desc", __('Issues'), array('class' => 'button button-silver', 'title' => __('Show all issues for this release'))); ?>
