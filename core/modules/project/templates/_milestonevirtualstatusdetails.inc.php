<?php if ($milestone instanceof \thebuggenie\core\entities\Milestone): ?>
    ...
    <div class="milestone_percentage" title="<?php echo __('%percentage % completed', array('%percentage' => $milestone->getPercentComplete($allowed_status_ids))); ?>">
        <div class="filler" id="milestone_<?php echo $milestone->getID(); ?>_percentage_filler" style="width: <?php echo $milestone->getPercentComplete($allowed_status_ids); ?>%;" title="<?php echo __('%percentage completed', array('%percentage' => $milestone->getPercentComplete($allowed_status_ids).'%')); ?>"></div>
    </div>
<?php else: ?>
