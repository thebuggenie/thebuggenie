<div id="milestone_<?php echo $milestone->getID(); ?>" class="milestone_box <?php echo ($milestone->isVisibleRoadmap()) ? ' available' : ' unavailable'; ?> <?php echo ($milestone->isReached()) ? 'closed' : 'open'; ?>" data-milestone-id="<?php echo $milestone->getID(); ?>">
    ...
    <?php
        if (isset($board)) {
            include_component('project/milestoneboxheader', compact('milestone', 'include_counts', 'include_buttons', 'board'));
        } else {
            include_component('project/milestoneboxheader', compact('milestone', 'include_counts', 'include_buttons'));
        }
    ?>
</div>
