<?php $user_id = isset($user_id) ? $user_id : 0; ?>
<?php $team_id = isset($team_id) ? $team_id : 0; ?>
<?php if (count($permissions_list) > 0): ?>
    <?php foreach ($permissions_list as $permission_key => $permission): ?>
        ...
        <?php else: ?>
            ...
            <li>
                <?php if ((!isset($user_id) || !$user_id) && (!isset($team_id) || !$team_id)): ?>
                    ...
                    <?php endif; ?>
                <?php elseif (isset($user_id) && $user_id): ?>
                    <?php $user = \thebuggenie\core\entities\User::getB2DBTable()->selectById($user_id); ?>
                ...
                <?php elseif (isset($team_id) && $team_id): ?>
                    <?php $team = \thebuggenie\core\entities\Team::getB2DBTable()->selectById($team_id); ?>
                    <div style="float: right;"><?php include_component('configuration/permissionsinfoitem', array('key' => $permission_key, 'target_id' => $current_target_id, 'type' => 'team', 'mode' => $mode, 'item_id' => $team->getID(), 'item_name' => urlencode($team->getName()), 'module' => $module, 'access_level' => $access_level)); ?></div>
                <?php endif; ?>
                <?php if(array_key_exists('details', $permission) && count($permission['details']) > 0): ?>
                    <a href="javascript:void(0);" onclick="TBG.Config.Permissions.getOptions('<?php echo make_url('configure_permissions_get_permissions', array('base_id' => $base_id . $permission_key, 'user_id' => (int) $user_id, 'team_id' => (int) $team_id, 'permissions_list' => $permission_key, 'user_id' => $user_id, 'team_id' => $team_id, 'mode' => $mode, 'target_id' => $current_target_id, 'target_module' => $module)); ?>', '<?php echo $base_id; ?>_<?php echo $permission_key; ?>_details');" style="float: left; margin-right: 5px;" title="<?php echo __('More fine-tuned permissions are available. Click to see them.'); ?>"><?php echo image_tag('expand_small.png'); ?></a>
                <?php endif; ?>
                <?php if ((!isset($user_id) || !$user_id) && (!isset($team_id) || !$team_id)): ?>
                    <a href="javascript:void(0);" onclick="<?php if(array_key_exists('details', $permission) && count($permission['details']) > 0): ?>$('<?php echo $base_id; ?>_<?php echo $permission_key; ?>_details').hide();<?php endif; ?>TBG.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'permissions', 'permission_key' => $permission_key, 'user_id' => $user_id, 'team_id' => $team_id, 'mode' => $mode, 'target_id' => $current_target_id, 'target_module' => $module, 'access_level' => $access_level)); ?>');" class="permission_description"><?php echo $permission['description']; ?></a>
                <?php else: ?>
