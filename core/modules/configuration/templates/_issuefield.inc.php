<li id="item_option_<?php echo $type; ?>_<?php echo $item->getID(); ?>" style="clear: both; height: 24px;">
    <div id="item_option_<?php echo $type; ?>_<?php echo $item->getID(); ?>_content">
        ...
        <div class="button-group" style="float: right;">
            <a href="javascript:void(0);" class="button button-icon button-silver" onclick="$('item_option_<?php echo $type; ?>_<?php echo $item->getID(); ?>_content').hide();$('edit_item_option_<?php echo $item->getID(); ?>').show();$('<?php echo $type; ?>_<?php echo $item->getID(); ?>_name_input').focus();" title="<?php echo __('Edit this item'); ?>"><?php echo fa_image_tag('edit'); ?></a>
            <a href="javascript:void(0);" class="button button-icon button-silver" onclick="TBG.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'issuefield_permissions', 'item_id' => $item->getID(), 'item_key' => $item->getPermissionsKey(), 'access_level' => $access_level)); ?>');" title="<?php echo __('Set permissions for this item'); ?>"><?php echo fa_image_tag('user-lock'); ?></a>
            <?php if ($item->canBeDeleted()): ?>
                <a href="javascript:void(0);" class="button button-icon button-silver" onclick="TBG.Main.Helpers.Dialog.show('<?php echo __('Really delete %itemname?', array('%itemname' => $item->getName())); ?>', '<?php echo __('Are you really sure you want to delete this item?'); ?>', {yes: {click: function() {TBG.Config.Issuefields.Options.remove('<?php echo make_url('configure_issuefields_delete', array('type' => $type, 'id' => $item->getID())); ?>', '<?php echo $type; ?>', <?php echo $item->getID(); ?>); TBG.Main.Helpers.Dialog.dismiss(); }}, no: {click: TBG.Main.Helpers.Dialog.dismiss}});" id="delete_<?php echo $item->getID(); ?>_link"><?php echo fa_image_tag('trash-alt', [], 'far'); ?></a>
            <?php else: ?>
        ...
        <form accept-charset="<?php echo \thebuggenie\core\framework\Context::getI18n()->getCharset(); ?>" action="<?php echo make_url('configure_issuefields_edit', array('type' => $type, 'id' => $item->getID())); ?>" onsubmit="TBG.Config.Issuefields.Options.update('<?php echo make_url('configure_issuefields_edit', array('type' => $type, 'id' => $item->getID())); ?>', '<?php echo $type; ?>', <?php echo $item->getID(); ?>);return false;" id="edit_<?php echo $type; ?>_<?php echo $item->getID(); ?>_form">
            <table style="width: 100%; table-layout: fixed;" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <?php endif; ?>
                    <td>
                        <input type="text" name="name" id="<?php echo $type; ?>_<?php echo $item->getID(); ?>_name_input" style="width: calc(100% - 20px);" value="<?php echo $item->getName(); ?>">
                    </td>
