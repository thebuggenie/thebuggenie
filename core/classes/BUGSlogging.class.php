<?php
	...
	class BUGSlogging
	{
		...
		protected static $_to_screen = false;
		...
		public static function log($message, $category = 'main', $level = 1)
		{
			...
			if (self::$_logonajaxcalls || !(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) || (isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"] == '')))
			{
				...
				{
					echo '[';
					echo (($load_time = BUGScontext::getLoadtime()) >= 1) ? round($load_time, 2) . ' seconds' : round(($load_time * 1000), 3) . ' ms'; 
					echo "] [".strtoupper(self::getLevelName($level))."] [{$category}] {$message}<br>";
				}
