    ...
    class TBGRouting
    {
        ...
        public function hasCachedRoutes()
        {
            if ($this->has_cached_routes === null)
            {
                ...
                {
                    $this->has_cached_routes = TBGContext::getCache()->has(TBGCache::KEY_ROUTES_CACHE);
        ...
        {
            TBGContext::getCache()->fileAdd(TBGCache::KEY_ROUTES_CACHE, $this->getRoutes());
            TBGContext::getCache()->add(TBGCache::KEY_ROUTES_CACHE, $this->getRoutes());
        }
        ...
        {
            TBGLogging::log("URL is '".htmlentities($url, ENT_COMPAT, 'utf-8')."'", 'routing');
            // an URL should start with a '/', mod_rewrite doesn't respect that, but no-mod_rewrite version does.
            if (mb_strlen($url) == 0 || '/' != $url[0])
            {
                $url = '/'.$url;
            }
            TBGLogging::log("URL is now '".htmlentities($url, ENT_COMPAT, 'utf-8')."'", 'routing');
