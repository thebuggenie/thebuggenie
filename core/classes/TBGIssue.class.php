<?php
    ...
    class TBGIssue extends TBGChangeableItem implements TBGMentionableProvider
    {
        ...
        public static function findIssues($filters = array(), $results_per_page = 30, $offset = 0, $groupby = null, $grouporder = null, $sortfields = array(TBGIssuesTable::LAST_UPDATED => 'asc'))
        {
            $issues = array();
...
//            }
        ...
        {
            $issuetype_id = ($issuetype instanceof TBGIssuetype) ? $issuetype->getID() : $issuetype;
        ...
        public function countChildIssues()
        {
            ...
            {
                return TBGIssueRelationsTable::getTable()->countChildIssues($this->getID());
            }
        }
            ...
            if ($this->getProject()->isTasksEnabled())
            {
                if ($this->_tasks == null)
                {
                    ...
                    if ($res = \b2db\Core::getTable('TBGIssueTasksTable')->getByIssueID($this->getID()))
                    {
                        ...
                        {
                            $this->_tasks[$row->get(TBGIssueTasksTable::ID)] = TBGContext::factory()->TBGIssue($row->get(TBGIssueTasksTable::ID), $row);
                        }
                    }
                }
            }
        ...
        {
            return htmlentities($this->_shortname, ENT_COMPAT, TBGContext::getI18n()->getCharset());
        }
        ...
        public function getCustomField($key)
        {
            ...
            if (property_exists($this, $var_name))
            {
                ...
                elseif ($this->$var_name && $customtype->hasPredefinedOptions() && !$this->$var_name instanceof TBGIdentifiable)
                {
                    try
                    {
                        switch ($customtype->getType())
                        {
                            case TBGCustomDatatype::EDITIONS_CHOICE:
                                $this->$var_name = TBGEditionsTable::getTable()->selectById($this->$var_name);
                                break;
                            case TBGCustomDatatype::COMPONENTS_CHOICE:
                                $this->$var_name = TBGComponentsTable::getTable()->selectById($this->$var_name);
                                break;
                            case TBGCustomDatatype::RELEASES_CHOICE:
                                $this->$var_name = TBGBuildsTable::getTable()->selectById($this->$var_name);
                                break;
                            case TBGCustomDatatype::MILESTONE_CHOICE:
                                $this->$var_name = TBGMilestonesTable::getTable()->selectById($this->$var_name);
                                break;
                            case TBGCustomDatatype::CLIENT_CHOICE:
                                $this->$var_name = TBGClientsTable::getTable()->selectById($this->$var_name);
                                break;
                        }
                    }
                    catch (Exception $e) { }
        ...
        {
            TBGIssuesTable::getTable()->touchIssue($this->getID());
        }
                        ...
                        try
                        {
                            switch ($customdatatype->getType())
                            {
                                ...
                                case TBGCustomDatatype::STATUS_CHOICE:
                                    ...
                                    break;
                                case TBGCustomDatatype::USER_CHOICE:
                                    $option_object = TBGContext::factory()->TBGUser($this->getCustomField($key));
                                    break;
                                case TBGCustomDatatype::TEAM_CHOICE:
                                    $option_object = TBGContext::factory()->TBGTeam($this->getCustomField($key));
                                    break;
                                case TBGCustomDatatype::CLIENT_CHOICE:
                                    $option_object = TBGClientsTable::getTable()->selectById($this->getCustomField($key));
                                    break;
                            }
                        }
                        catch (Exception $e) {}
                    ...
                    if ($original_value != $compare_value)
                    {
                        switch ($property)
                        {
                            case '_title':
                                ...
                                break;
                            case '_shortname':
                                $this->addLogEntry(TBGLogTable::LOG_ISSUE_UPDATE_SHORTNAME, TBGContext::getI18n()->__("Issue label updated"), $original_value, $compare_value);
                                break;
                            case '_description':
                            ...
                            case '_issuetype':
                                if ($original_value != 0)
                                {
                                    $old_name = ($old_item = TBGIssuetype::getB2DBTable()->selectById($original_value)) ? $old_item->getName() : TBGContext::getI18n()->__('Unknown');
                                }
                                else
                                {
        ...
        protected function _postSave($is_new)
        {
            ...
            if (!$is_new)
            {
                ...
                if (count($related_issues_to_save))
                {
                    ...
                    {
                        $related_issue = TBGContext::factory()->TBGIssue((int) $i_id);
                        $related_issue->save();
                    }
                }
            }
