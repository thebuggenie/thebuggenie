<?php
	...
	class BUGScustomdatatypeoption extends BUGSdatatypebase
	{
		...
		public static function getAllByKey($key)
		{
			if (!array_key_exists($key, self::$_items))
			{
				...
				if ($items = B2DB::getTable('B2tCustomFieldOptions')->getAllByKey($key))
				{
					...
					{
						self::$_items[$key][$row_id] = BUGSfactory::BUGScustomdatatypeoptionLab($row_id, $row);
					}
				}
			}
		...
		{
			if ($type == BUGScustomdatatype::DROPDOWN_CHOICE_TEXT_ICON)
			{

			}
			elseif (in_array($type, array(BUGScustomdatatype::DROPDOWN_CHOICE_TEXT_COLORED, BUGScustomdatatype::DROPDOWN_CHOICE_TEXT_COLOR)))
			{
			...
			return BUGSfactory::BUGScustomdatatypeoptionLab($res->getInsertID());
		}
