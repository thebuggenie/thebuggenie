<?php
	 ...
	 * @subpackage main
	 */
	class BUGSedition extends BUGSversionitem 
	{
		/**
		 * The project
		 *
		...
		public static function createNew($e_name, $p_id, $e_id = null)
		{
			...
			$crit->addInsert(B2tEditions::SCOPE, BUGScontext::getScope()->getID());
			$crit->addInsert(B2tEditions::DESCRIPTION, '');
			$res = B2DB::getTable('B2tEditions')->doInsert($crit);
			
			if ($e_id === null) $e_id = $res->getInsertID();
			
			BUGScontext::setPermission("b2editionaccess", $e_id, "core", 0, BUGScontext::getUser()->getGroup()->getID(), 0, true);
			return BUGSfactory::editionLab($e_id);
		}
		...
		public static function getAllByProjectID($project_id)
		{
			...
			if (!array_key_exists($project_id, self::$_editions))
			{
				...
				if ($res = B2DB::getTable('B2tEditions')->getByProjectID($project_id))
				{
					...
					{
						$edition = BUGSfactory::editionLab($row->get(B2tEditions::ID), $row);
		...
		public function __construct($e_id, $row = null)
		{
			...
			{
				$crit = new B2DBCriteria();
				$crit->addWhere(B2tEditions::SCOPE, BUGScontext::getScope()->getID());
		...
		public function doPopulateComponents()
		{
			...
			if ($res->count() > 0)
			{
				while ($row = $res->getNextRow())
				{
					$this->_components[$row->get(B2tEditionComponents::COMPONENT)] = BUGSfactory::componentLab($row->get(B2tEditionComponents::COMPONENT));
				}
			}
		...
		{
			if ($c_id instanceof BUGScomponent)
			{
				$c_id = $c_id->getID();
			}
		 ...
		 *
		 * @param BUGScomponent|integer $c_id
		 */
		public function addComponent($c_id)
		{
			...
			$crit->addInsert(B2tEditionComponents::SCOPE, BUGScontext::getScope()->getID());
		...
		public function removeComponent($c_id)
		{
			...
			$crit->addWhere(B2tEditionComponents::SCOPE, BUGScontext::getScope()->getID());
		...
		public function getLatestBuild()
		{
			...
			if ($res->count() > 0)
			{
				while ($row = $res->getNextRow())
				{
					if (BUGScontext::getUser()->hasPermission('b2buildaccess', $row->get(B2tBuilds::ID), 'core'))
					{
						return(BUGSfactory::buildLab($row->get(B2tBuilds::ID)));
					}
				}
			}
			...
			if ($res->count() > 0)
			{
				while ($row = $res->getNextRow())
				{
					if (!isset($this->_builds[$row->get(B2tBuilds::ID)]))
					{
						if (BUGScontext::getUser()->hasPermission('b2buildaccess', $row->get(B2tBuilds::ID), 'core'))
						{
							$this->_builds[$row->get(B2tBuilds::ID)] = BUGSfactory::buildLab($row->get(B2tBuilds::ID), $row);
						}
					}
				}
			}
		...
		public function addAssignee($assignee, $role)
		{
			...
			if (!$res instanceof B2DBRow)
			{
				...
				$crit->addInsert(B2tEditionAssignees::SCOPE, BUGScontext::getScope()->getID());
				try
				{
		...
		{
			if ($this->_lead_type == BUGSidentifiableclass::TYPE_USER)
			{
				return BUGSfactory::userLab($this->_lead_by);
			}
			elseif ($this->_lead_type == BUGSidentifiableclass::TYPE_TEAM)
			{
				return BUGSfactory::teamLab($this->_lead_by);
			}
			return null;
		}
		...
		{
			return ($this->getLeadBy() instanceof BUGSidentifiable) ? true : false;
		}
		...
		{
			if ($this->_owned_type == BUGSidentifiableclass::TYPE_USER)
			{
				return BUGSfactory::userLab($this->_owned_by);
			}
			elseif ($this->_owned_type == BUGSidentifiableclass::TYPE_TEAM)
			{
				return BUGSfactory::teamLab($this->_owned_by);
			}
			return null;
		}
		...
		{
			return ($this->getOwnedBy() instanceof BUGSidentifiable) ? true : false;
		}
		...
		{
			if ($this->_qa_type == BUGSidentifiableclass::TYPE_USER)
			{
				return BUGSfactory::userLab($this->_qa);
			}
			elseif ($this->_qa_type == BUGSidentifiableclass::TYPE_TEAM)
			{
				return BUGSfactory::teamLab($this->_qa);
			}
			return null;
		}
		...
		{
			return ($this->getQA() instanceof BUGSidentifiable) ? true : false;
		}
		...
		public function delete()
		{
			...
			$crit = new B2DBCriteria();
			$crit->addWhere(B2tEditionAssignees::EDITION_ID, $this->getID());
			$crit->addWhere(B2tEditionAssignees::SCOPE, BUGScontext::getScope()->getID());
		...
		{
			return BUGScontext::getUser()->hasPermission('b2editionaccess', $this->getID(), 'core');			
		}
