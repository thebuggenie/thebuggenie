<?php
    ...
    class TBGContext
    {
        ...
        public static function autoloadNamespace($namespace, $path)
        {
            $path = realpath($path);
            if (!file_exists($path))
                throw new Exception("Cannot add {$path} to autoload, since the path doesn't exist");
            self::$_classpaths[$namespace] = $path;
        }

        public static function autoloadModuleClasses($path)
        {
            $realpath = realpath($path);
            if (!file_exists($realpath))
                throw new \Exception('Invalid module path: "'.$path.'"');

            if (file_exists($realpath . DS . 'actions.class.php'))
                require_once $realpath . DS . 'actions.class.php';

            if (file_exists($realpath . DS . 'actioncomponents.class.php'))
                require_once $realpath . DS . 'actioncomponents.class.php';
        }
        ...
        protected static function _setTBGPath()
        {
            ...
            {
                self::$_tbgpath = str_replace("\\", "/", self::$_tbgpath); /* Windows adds a \ to the URL which we don't want */
            }
            if (self::$_tbgpath[strlen(self::$_tbgpath) - 1] != '/')
                self::$_tbgpath .= '/';
        }
        ...
        public static function initialize()
        {
            try
            {
                ...
                TBGLogging::log('Debug mode: ' . ((self::$_debug_mode) ? 'yes' : 'no'));
        ...
        {
            TBGLogging::log('Loading routes from routing file', 'routing');
            ...
            TBGLogging::log('...done (loading routes from routing file)', 'routing');
        }
        ...
        public static function loadInternalModules()
        {
            ...
            {
                TBGLogging::log('Loading cached modules');
                self::$_internal_modules = $modules;
            }

            TBGLogging::log('...done (loading internal modules)');
        }
        ...
        public static function loadModules()
        {
            ...
            {
                self::$_modules = TBGModulesTable::getTable()->getAllNames();
                return;
            }

            self::$_modules = TBGModulesTable::getTable()->getAll();
                ...
                if ($content === null)
                {
                    ...
                    elseif (!$action_retval)
                    {
                        ...
                        if (!isset($templateName) || !file_exists($templateName))
                        {
                            ...
                            TBGLogging::log('Trying to load file "' . $templateName . '"', 'core', TBGLogging::LEVEL_FATAL);
                ...
                if (($route = self::getRouting()->getRouteFromUrl(self::getRequest()->getParameter('url', null, false))) || self::isInstallmode())
                {
                    ...
                    if (!in_array($route['module'], self::$_internal_modules))
                    {
                        if (is_dir(THEBUGGENIE_MODULES_PATH . $route['module']))
                        {
                            ...
                            {
                                throw new TBGActionNotFoundException('The ' . $route['module'] . ' module is missing the Actions.php file, containing all the module actions');
                            }
                        }
                        else
                        {
