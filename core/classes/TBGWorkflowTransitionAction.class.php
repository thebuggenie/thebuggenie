    ...
    class TBGWorkflowTransitionAction extends TBGIdentifiableScopedClass
    {
        ...
        protected static function _populateAvailableActions()
        {
            if (self::$_available_actions === null)
            {
                ...
                $event = new TBGEvent('core', 'TBGWorkflowTransitionAction::getAvailableTransitionActions', null, array(), $initial_list);
                $event->trigger();
            ...
            switch ($this->_action_type)
            {
                ...
                case self::ACTION_SET_STATUS:
                    if ($this->getTargetValue())
                        $issue->setStatus(TBGStatus::getB2DBTable()->selectById((int) $this->getTargetValue()));
                    else
                        $issue->setStatus($request['status_id']);
                    break;
                case self::ACTION_SET_MILESTONE:
                    if ($this->getTargetValue())
                        $issue->setMilestone(TBGMilestone::getB2DBTable()->selectById((int) $this->getTargetValue()));
                    else
                ...
                case self::ACTION_SET_PRIORITY:
                    if ($this->getTargetValue())
                        $issue->setPriority(TBGPriority::getB2DBTable()->selectById((int) $this->getTargetValue()));
                    else
                ...
                case self::ACTION_SET_RESOLUTION:
                    if ($this->getTargetValue())
                        $issue->setResolution(TBGResolution::getB2DBTable()->selectById((int) $this->getTargetValue()));
                    else
                ...
                case self::ACTION_SET_REPRODUCABILITY:
                    if ($this->getTargetValue())
                        $issue->setReproducability(TBGReproducability::getB2DBTable()->selectById((int) $this->getTargetValue()));
                    else
                ...
                default:
                    ...
                    {
                        $event = new TBGEvent('core', 'TBGWorkflowTransitionAction::perform', $issue, array('request' => $request));
