<?php
	 ...
	 * @subpackage main
	 */
	class BUGSbuild extends BUGSversionitem 
	{
		/**
		 * This builds edition
		 *
		...
		public static function getByProjectID($project_id)
		{
			...
			if (!array_key_exists($project_id, self::$_project_builds))
			{
				...
				if ($res = B2DB::getTable('B2tBuilds')->getByProjectID($project_id))
				{
					...
					{
						$build = BUGSfactory::buildLab($row->get(B2tBuilds::ID), $row);
		...
		public static function createNew($name, $project = null, $edition = null, $ver_mj = 0, $ver_mn = 0, $ver_rev = 1, $b_id = null)
		{
			...
			BUGScontext::setPermission("b2buildaccess", $b_id, "core", 0, BUGScontext::getUser()->getGroup()->getID(), 0, true);
			
			return BUGSfactory::buildLab($b_id);
		}
		...
		public function __construct($b_id, $row = null)
		{
			...
			if ($row instanceof B2DBRow)
			{
				...
				{
					$this->_edition = BUGSfactory::editionLab($row->get(B2tBuilds::EDITION), $row);
				}
				elseif ($row->get(B2tBuilds::PROJECT))
				{
					$this->_project = BUGSfactory::projectLab($row->get(B2tBuilds::PROJECT), $row);
				}
			}
		}
		...
		{
			return BUGScontext::getUser()->hasPermission('b2buildaccess', $this->getID(), 'core');
		}
