            ...
            <?php if (\thebuggenie\core\framework\Context::getRouting()->getCurrentRouteName() != 'login_page'): ?>
                <?php if (\thebuggenie\core\framework\Event::createNew('core', 'header_usermenu_decider')->trigger()->getReturnValue() !== false): ?>
                    <ul class="tab_menu_dropdown user_menu_dropdown" id="user_menu">
                        <?php if ($tbg_user->isGuest()): ?>
                            <li><a href="javascript:void(0);" onclick="TBG.Main.Login.showLogin('regular_login_container');"><?php echo fa_image_tag('sign-in').__('Login'); ?></a></li>
                            <?php if (\thebuggenie\core\framework\Settings::isRegistrationAllowed()): ?>
                                <li><a href="javascript:void(0);" onclick="TBG.Main.Login.showLogin('register');"><?php echo fa_image_tag('user-plus').__('Register'); ?></a></li>
                            <?php endif; ?>
