<?php
	...
	else
	{
		...
		{
			if (TBGContext::getRequest()->getParameter('max_file_size'))
			{
				$max_file_size = (int) TBGContext::getRequest()->getParameter('max_file_size', null, false);
				if ($max_file_size > (int) ini_get('upload_max_filesize')) $max_file_size = (int) ini_get('upload_max_filesize');
				TBGSettings::saveSetting('max_file_size', $max_file_size);
			}
			if (TBGContext::getRequest()->getParameter('enable_uploads'))
			{
				TBGSettings::saveSetting('enable_uploads', TBGContext::getRequest()->getParameter('enable_uploads'));
			}
			if (TBGContext::getRequest()->getParameter('enable_guest_uploads'))
			{
				TBGSettings::saveSetting('enable_guest_uploads', TBGContext::getRequest()->getParameter('enable_guest_uploads'));
			}
			if (TBGContext::getRequest()->getParameter('uploads_blacklist'))
			{
				TBGSettings::saveSetting('uploads_blacklist', TBGContext::getRequest()->getParameter('uploads_blacklist'));
			}
			if (TBGContext::getRequest()->getParameter('uploads_filetypes'))
			{
				TBGSettings::saveSetting('uploads_filetypes', TBGContext::getRequest()->getParameter('uploads_filetypes'));
			}
		}
		?>
			...
			<td style="padding-right: 10px;">
				<table class="configstrip" cellpadding=0 cellspacing=0>
					...
					<tr>
						...
						<?php echo __('From here you can manage file uploads.') . __('To find out more about what each setting does, please refer to the %tbg_online_help%', array('%tbg_online_help%' => tbg_helpBrowserHelper('uploadsettings', 'The Bug Genie online help'))); ?>
						</td>
					</tr>
				</table>
				</td>
		...
		<table style="width: auto" cellpadding=0 cellspacing=0>
			<tr>
				...
				<td style="width: 250px;">
					<select name="enable_uploads" style="width: 100%"<?php echo ($access_level != 'full') ? ' disabled' : ''; ?>>
						<option value=1 <?php echo (TBGSettings::get('enable_uploads') == 1) ? ' selected' : ''; ?>><?php echo __('Yes, allow file uploads'); ?></option>
						<option value=0 <?php echo (TBGSettings::get('enable_uploads') == 0) ? ' selected' : ''; ?>><?php echo __('No, don\'t allow file uploads'); ?></option>
					</select>
				</td>
			...
			<tr>
				...
				<td style="width: 250px;">
					<select name="enable_guest_uploads" style="width: 100%"<?php echo ($access_level != 'full') ? ' disabled' : ''; ?>>
						<option value=1 <?php echo (TBGSettings::get('enable_guest_uploads') == 1) ? ' selected' : ''; ?>><?php echo __('Yes, let anonymous users upload files'); ?></option>
						<option value=0 <?php echo (TBGSettings::get('enable_guest_uploads') == 0) ? ' selected' : ''; ?>><?php echo __('No, only allow registered users to upload files'); ?></option>
					</select>
				</td>
			...
			<tr>
				...
				<td style="width: 250px;"><input type="text" name="max_file_size" value="<?php echo TBGSettings::get('max_file_size'); ?>" style="width: 25%; text-align: right;">&nbsp;<b>MB</b></td>
			...
			<tr>
				...
				<td style="width: 250px;">
					<select name="uploads_blacklist" style="width: 100%"<?php echo ($access_level != 'full') ? ' disabled' : ''; ?>>
						<option value=1 <?php echo (TBGSettings::get('uploads_blacklist') == 1) ? ' selected' : ''; ?>><?php echo __('Blacklist - allow everything but these filetypes'); ?></option>
						<option value=0 <?php echo (TBGSettings::get('uploads_blacklist') == 0) ? ' selected' : ''; ?>><?php echo __('Whitelist - allow nothing but these filetypes'); ?></option>
					</select>
				</td>
			...
			<tr>
				...
				<td style="width: 250px;"><input type="text" name="uploads_filetypes" value="<?php echo TBGSettings::get('uploads_filetypes'); ?>"></td>
