<div id="tab_vcs_pane"<?php if ($selected_tab != 'vcs'): ?> style="display: none;"<?php endif; ?>>
	...
	<div id="vcs_config_menu_panes">
		<div id="tab_vcs_general_pane">
			...
			<div class="left">
				<div class="tab_header"><?php echo __('Web hook URLs'); ?></div>
				<div class="tab_content">
					<?php echo __('If you are using the HTTP access method, use these project-specific URLs in the hooks. Hooks for web serves such as github will need to be added to repository configuration in the service, see the help for details.'); ?>
					<div class="rounded_box lightgrey" style="margin-top: 10px">
						<div class="header"><?php echo __('Standard HTTP Access URL (for standard hooks):'); ?></div><code><?php echo TBGContext::getURLhost().make_url('normalcheckin', array('project_key' => $project->getKey())); ?>?passkey=<i>your passkey here</i></code>
					</div>
					<div class="rounded_box lightgrey" style="margin-top: 10px">
						<div class="header">github <?php echo __('(set as a post-recieve URL)'); ?>:</div><code><?php echo TBGContext::getURLhost().make_url('githubcheckin', array('project_key' => $project->getKey())); ?>?passkey=<i>your passkey here</i></code>
					</div>
					<div class="rounded_box lightgrey" style="margin-top: 10px">
						<div class="header">gitorious (<?php echo __('Locally hosted - see gitorious documentation for setup instructions'); ?>):</div><code><?php echo TBGContext::getURLhost().make_url('gitoriouscheckin', array('project_key' => $project->getKey())); ?>?passkey=<i>your passkey here</i></code>
					</div>
				</div>
			</div>
		</div>
		...
		<div id="tab_vcs_browser_pane" style="display: none">
			<table style="clear: both; width: 780px;" class="padded_table" cellpadding=0 cellspacing=0>
				...
				<tr>
					...
					<td style="width: 580px; position: relative;">
						<select name="browser_type" id="browser_type" onchange="if ($('browser_type').getValue() == 'other') { $('vcs_custom_urls').show(); } else { $('vcs_custom_urls').hide(); }">
				...
				<tr>
					<td class="config_explanation" colspan="2"><?php echo __('Choosing the repository browser will use will automatically set the URLs for the pages The Bug Genie needs to access. If you choose Custom you can manually set these URLs.'); ?></td>
				</tr>
				<tr>
					<td colspan="2" id="vcs_custom_urls" style="display: none;">
						...
						<div class="rounded_box lightgrey left" style="margin-top: 10px">
							<div class="header"><?php echo __('In the following fields, these parameters will be replaced with real values when links are generated:'); ?></div>
							<ul>
