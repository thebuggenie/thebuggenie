<?php
	...
	switch (TBGContext::getModule('vcs_integration')->getSetting('web_type_' . $projectId))
	{
		case 'viewvc':
			...
			$link_old = $web_path . '/' . '?root=' . $web_repo . '&amp;view=rev&amp;revision=' . $revision; 
			break;
		case 'viewvc_repo':
			$link_rev = $web_path . '/' . '?view=rev&amp;revision=' . $revision;
			$link_old = $web_path . '/' . '?view=rev&amp;revision=' . $revision; 
			break;
		case 'websvn':
			$link_rev = $web_path . '/revision.php?repname=' . $web_repo . '&amp;isdir=1&amp;rev=' . $revision;
			$link_old = $web_path . '/revision.php?repname=' . $web_repo . '&amp;isdir=1&amp;rev=' . $revision; 
			break;
		case 'websvn_mv':
			$link_rev = $web_path . '/' . '?repname=' . $web_repo . '&amp;op=log&isdir=1&amp;rev=' . $revision;
			$link_old = $web_path . '/' . '?repname=' . $web_repo . '&amp;op=log&isdir=1&amp;rev=' . $revision; 
			break;
		case 'loggerhead':
			$link_rev = $web_path . '/' . $web_repo . '/revision/' . $revision;
			$link_old = $web_path . '/' . $web_repo . '/revision/' . $revision; 
			break;
		case 'gitweb':
			$link_rev = $web_path . '/' . '?p=' . $web_repo . ';a=commitdiff;h=' . $revision;
			$link_old = $web_path . '/' . '?p=' . $web_repo . ';a=commitdiff;h=' . $revision; 
			break;
		case 'cgit':
			$link_rev = $web_path . '/' . $web_repo . '/commit/?id=' . $revision;
			$link_old = $web_path . '/' . $web_repo . '/commit/?id=' . $revision; 
			break;
		case 'hgweb':
			$revision = explode(':', $revision);
			$link_rev = $web_path . '/' . $web_repo . '/rev/' . $revision[1];
			$link_old = $web_path . '/' . $web_repo . '/rev/' . $revision[1]; 
			break;
		case 'github':
			$link_rev = 'http://github.com/' . $web_repo . '/commit/' . $revision;
			$link_old = 'http://github.com/' . $web_repo . '/commit/' . $revision; 
			break;
		case 'gitorious':
...
<div class="comment" id="commit_<?php echo $id; ?>">
	<div style="position: relative; overflow: visible; padding: 5px;" id="commit_view_<?php echo $id; ?>" class="comment_main">
		<div id="commit_<?php echo $id; ?>_header" class="commentheader">
			...
			<div class="commentdate" id="commit_<?php echo $id; ?>_date"><?php echo tbg_formattime($date, 12); ?> - <?php echo __('Preceeded by %prev%', array('%prev%' => "<a href=".$link_old.">".$files[0][3]."</a>"))?></div>
		</div>
		
		<div class="commentbody article commit_main" id="commit_<?php echo $id; ?>_body">
			<div class="commit_expander">
				<a href="javascript:void(0);" style="padding-right: 5px;" id="checkin_expand_<?php echo $id; ?>" onclick="$('checkin_details_<?php echo $id; ?>').show(); $('checkin_expand_<?php echo $id; ?>').hide(); $('checkin_collapse_<?php echo $id; ?>').show();"><?php echo image_tag('expand.png'); ?> <?php echo __("Show more details"); ?></a>
				<a href="javascript:void(0);" style="display: none; padding-right: 5px;" id="checkin_collapse_<?php echo $id; ?>" onclick="$('checkin_details_<?php echo $id; ?>').hide(); $('checkin_expand_<?php echo $id; ?>').show(); $('checkin_collapse_<?php echo $id; ?>').hide();"><?php echo image_tag('collapse.png'); ?> <?php echo __("Hide details"); ?></a>
			</div>
