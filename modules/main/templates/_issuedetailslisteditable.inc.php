<div id="viewissue_left_box_issuetype">
	<div id="issuetype_header" class="hoverable <?php if ($issue->isIssuetypeChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isIssuetypeMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<?php if ($issue->isEditable() && $issue->canEditIssuetype()): ?>
			<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'issuetype')); ?>', 'issuetype');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
			...
			<table cellpadding="0" cellspacing="0">
				<?php foreach ($issuetypes as $issuetype): ?>
					<tr>
						...
						<td style="padding-left: 5px;"><a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'issuetype', 'issuetype_id' => $issuetype->getID())); ?>', 'issuetype');"><?php echo $issuetype->getName(); ?></a></td>
					</tr>
				<?php endforeach; ?>
			</table>
...
<div id="viewissue_left_box_status">
	<div id="status_header" class="hoverable <?php if ($issue->isStatusChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isStatusMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<?php if ($issue->isUpdateable() && $issue->canEditStatus()): ?>
			<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'status')); ?>', 'status');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
...
<?php if ($issue->isUpdateable() && $issue->canEditStatus()): ?>
	<div class="rounded_box white shadowed" id="status_change" style="display: none; width: 280px; position: absolute; z-index: 10001; margin: 5px 0 5px 0;">
		...
		<div class="xboxcontent" style="padding: 5px;">
			...
			<div class="dropdown_content">
				<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'status', 'status_id' => 0)); ?>', 'status');"><?php echo __('Clear the status'); ?></a><br>
			</div>
			<div class="dropdown_content">
				...
				<table cellpadding="0" cellspacing="0">
					<?php foreach ($statuses as $status): ?>
						...
						<tr>
							...
							<td style="padding-left: 5px;"><a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'status', 'status_id' => $status->getID())); ?>', 'status');"><?php echo $status->getName(); ?></a></td>
						</tr>
					<?php endforeach; ?>
				</table>
...
<dl class="viewissue_list" id="assigned_to_field">
	...
	<dd id="assigned_to_content" class="<?php if ($issue->isAssignedToChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isAssignedToMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<?php if ($issue->canEditAssignedTo()): ?>
			<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'assigned_to')); ?>', 'assigned_to');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
...
<dl class="viewissue_list" id="percent_complete_field"<?php if (!$issue->isPercentCompletedVisible()): ?> style="display: none;"<?php endif; ?>>
	...
	<dd id="percent_content" class="<?php if ($issue->isPercentCompletedChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isPercentCompletedMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<div style="width: 210px;">
			<?php if ($issue->canEditPercentage() && $issue->isEditable()): ?>
				<a href="javascript:void(0);" onclick="thebuggenie.events.updatePercent('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'percent')); ?>', 'percent');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
					...
					<tr id="percentage_tds">
						<?php for ($cc = 0; $cc <= 100; $cc++): ?>
							<td class="<?php if ($issue->getPercentCompleted() <= $cc): ?>percent_unfilled<?php else: ?>percent_filled<?php endif; ?>" style="font-size: 1px; width: 1%; height: 14px;">
								<a href="javascript:void(0);" onclick="thebuggenie.events.updatePercent('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'percent', 'percent' => $cc)); ?>', 'set');" title="<?php echo __('Set to %pct% percent', array('%pct%' => $cc)); ?>">&nbsp;</a>
							</td>
						<?php endfor; ?>
					</tr>
				</table>
...
<div style="margin: 0;<?php if (!$issue->isUserPainVisible()): ?> display: none;<?php endif; ?>" id="user_pain_field">
	<dl class="viewissue_list" id="pain_bug_type_field">
		...
		<dd id="pain_bug_type_content" class="<?php if ($issue->isPainBugTypeChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isPainBugTypeMerged()): ?> issue_detail_unmerged<?php endif; ?>">
			<?php if ($issue->isEditable() && $issue->canEditUserPain()): ?>
				<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'pain_bug_type')); ?>', 'pain_bug_type');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
	...
	<?php if ($issue->isEditable() && $issue->canEditUserPain()): ?>
		<div class="rounded_box white shadowed" id="pain_bug_type_change" style="display: none; width: 280px; position: absolute; z-index: 10001; margin: 5px 0 5px 0;">
			...
			<div class="xboxcontent" style="padding: 5px;">
				...
				<div class="dropdown_content">
					<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'pain_bug_type', 'pain_bug_type_id' => 0)); ?>', 'pain_bug_type');"><?php echo __('Clear bug type'); ?></a><br>
				</div>
				<div class="dropdown_content">
	...
	<dl class="viewissue_list" id="pain_likelihood_field">
		...
		<dd id="pain_likelihood_content" class="<?php if ($issue->isPainLikelihoodChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isPainLikelihoodMerged()): ?> issue_detail_unmerged<?php endif; ?>">
			<?php if ($issue->isEditable() && $issue->canEditUserPain()): ?>
				<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'pain_likelihood')); ?>', 'pain_likelihood');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
	...
	<?php if ($issue->isEditable() && $issue->canEditUserPain()): ?>
		<div class="rounded_box white shadowed" id="pain_likelihood_change" style="display: none; width: 280px; position: absolute; z-index: 10001; margin: 5px 0 5px 0;">
			...
			<div class="xboxcontent" style="padding: 5px;">
				...
				<div class="dropdown_content">
					<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'pain_likelihood', 'pain_likelihood_id' => 0)); ?>', 'pain_likelihood');"><?php echo __('Clear likelihood'); ?></a><br>
				</div>
				<div class="dropdown_content">
	...
	<dl class="viewissue_list" id="pain_effect_field">
		...
		<dd id="pain_effect_content" class="<?php if ($issue->isPainEffectChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isPainEffectMerged()): ?> issue_detail_unmerged<?php endif; ?>">
			<?php if ($issue->isEditable() && $issue->canEditUserPain()): ?>
				<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'pain_effect')); ?>', 'pain_effect');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
	...
	<?php if ($issue->isEditable() && $issue->canEditUserPain()): ?>
		<div class="rounded_box white shadowed" id="pain_effect_change" style="display: none; width: 280px; position: absolute; z-index: 10001; margin: 5px 0 5px 0;">
			...
			<div class="xboxcontent" style="padding: 5px;">
				...
				<div class="dropdown_content">
					<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'pain_effect', 'pain_effect_id' => 0)); ?>', 'pain_effect');"><?php echo __('Clear effect'); ?></a><br>
				</div>
				<div class="dropdown_content">
...
<dl class="viewissue_list" id="posted_by_field">
	...
	<dd id="posted_by_content" class="<?php if ($issue->isPostedByChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isPostedByMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<?php if ($issue->isEditable() && $issue->canEditPostedBy()): ?>
			<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'posted_by')); ?>', 'posted_by');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
...
<dl class="viewissue_list" id="owned_by_field">
	...
	<dd id="owned_by_content" class="<?php if ($issue->isOwnedByChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isOwnedByMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<?php if ($issue->isEditable() && $issue->canEditOwnedBy()): ?>
			<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'owned_by')); ?>', 'owned_by');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
...
<dl class="viewissue_list" id="estimated_time_field"<?php if (!$issue->isEstimatedTimeVisible()): ?> style="display: none;"<?php endif; ?>>
	...
	<dd id="estimated_time_content" class="<?php if ($issue->isEstimatedTimeChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isEstimatedTimeMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<?php if ($issue->isEditable() && $issue->canEditEstimatedTime()): ?>
			<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'estimated_time')); ?>', 'estimated_time');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
...
<?php if ($issue->isEditable() && $issue->canEditEstimatedTime()): ?>
	<div class="rounded_box white shadowed" id="estimated_time_change" style="display: none; width: 280px; position: absolute; z-index: 10001; margin: 5px 0 5px 0;">
		<b class="xtop"><b class="xb1"></b><b class="xb2"></b><b class="xb3"></b><b class="xb4"></b></b>
		<div class="xboxcontent" style="padding: 5px;">
			<form id="estimated_time_form" method="post" accept-charset="<?php echo TBGContext::getI18n()->getCharset(); ?>" action="" onsubmit="thebuggenie.events.setTimeField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'estimated_time')); ?>', 'estimated_time');return false;">
				<div class="dropdown_header"><?php echo __('Estimate this issue'); ?></div>
				<div class="dropdown_content">
					<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'estimated_time', 'value' => 0)); ?>', 'estimated_time');"><?php echo __('Clear current estimate'); ?></a><br>
				</div>
				<div class="dropdown_content">
...
<dl class="viewissue_list" id="spent_time_field"<?php if (!$issue->isSpentTimeVisible()): ?> style="display: none;"<?php endif; ?>>
	...
	<dd id="spent_time_content" class="<?php if ($issue->isSpentTimeChanged()): ?>issue_detail_changed<?php endif; ?><?php if (!$issue->isSpentTimeMerged()): ?> issue_detail_unmerged<?php endif; ?>">
		<?php if ($issue->isEditable() && $issue->canEditSpentTime()): ?>
			<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'spent_time')); ?>', 'spent_time');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
...
<?php if ($issue->isEditable() && $issue->canEditSpentTime()): ?>
	<div class="rounded_box white shadowed" id="spent_time_change" style="display: none; width: 280px; position: absolute; z-index: 10001; margin: 5px 0 5px 0;">
		<b class="xtop"><b class="xb1"></b><b class="xb2"></b><b class="xb3"></b><b class="xb4"></b></b>
		<div class="xboxcontent" style="padding: 5px;">
			<form id="spent_time_form" method="post" accept-charset="<?php echo TBGContext::getI18n()->getCharset(); ?>" action="" onsubmit="thebuggenie.events.setTimeField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'spent_time')); ?>', 'spent_time');return false;">
				<div class="dropdown_header"><?php echo __('Set time spent on this issue'); ?></div>
				<div class="dropdown_content">
					<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => 'spent_time', 'value' => 0)); ?>', 'spent_time');"><?php echo __('Clear time spent on this issue'); ?></a><br>
				</div>
				<div class="dropdown_content">
	...
	<dl class="viewissue_list" id="<?php echo $field; ?>_field"<?php if (!$info['visible']): ?> style="display: none;"<?php endif; ?>>
		...
		<dd id="<?php echo $field; ?>_content" class="<?php if ($info['changed']): ?>issue_detail_changed<?php endif; ?><?php if (!$info['merged']): ?> issue_detail_unmerged<?php endif; ?>">
			<?php if (array_key_exists('choices', $info) && isset($info['choices'])): ?>
				<a href="javascript:void(0);" onclick="thebuggenie.events.revertField('<?php echo make_url('issue_revertfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => $field)); ?>', '<?php echo $field; ?>');" title="<?php echo __('Undo this change'); ?>"><?php echo image_tag('undo.png', array('class' => 'undo')); ?></a>
	...
	<?php if (array_key_exists('choices', $info) && isset($info['choices'])): ?>
		<div class="rounded_box white shadowed" id="<?php echo $field; ?>_change" style="display: none; width: 280px; position: absolute; z-index: 10001; margin: 5px 0 5px 0;">
			...
			<div class="xboxcontent" style="padding: 5px;">
				...
				<div class="dropdown_content">
					<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => $field, $field . '_id' => 0)); ?>', '<?php echo $field; ?>');"><?php echo $info['clear']; ?></a><br>
				</div>
				<div class="dropdown_content">
					...
					<ul class="choices">
						<?php foreach ($info['choices'] as $choice): ?>
							...
							<li>
								<?php echo image_tag('icon_' . $field . '.png', array('style' => 'float: left; margin-right: 5px;')); ?><a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => $field, $field . '_id' => $choice->getID())); ?>', '<?php echo $field; ?>');"><?php echo $choice->getName(); ?></a>
							</li>
						<?php endforeach; ?>
					</ul>
			...
			<div class="xboxcontent" style="padding: 5px;">
				<div class="dropdown_header"><?php echo $info['change_header']; ?></div>
					...
					<?php if (array_key_exists('choices', $info)): ?>
						<a href="javascript:void(0);" onclick="thebuggenie.events.setField('<?php echo make_url('issue_setfield', array('project_key' => $issue->getProject()->getKey(), 'issue_id' => $issue->getID(), 'field' => $field, $field . '_value' => "")); ?>', '<?php echo $field; ?>');"><?php echo $info['clear']; ?></a><br>
						<ul class="choices">
							<?php foreach ($info['choices'] as $choice): ?>
...
<div id="more_actions" style="display: none;">
    ...
    <ul>
    	...
    	<li><a href="javascript:void(0);" onclick="thebuggenie.events.showFadedBackdrop('<?php echo make_url('get_partial_for_backdrop', array('key' => 'markasduplicate_issue', 'issue_id' => $issue->getID())); ?>');"><?php echo image_tag('icon_duplicate_issues.png', array('style' => 'float: left; margin-right: 5px;')); ?><?php echo __('Mark this issue as a duplicate of another'); ?></a></li>
    	<?php else: ?>
    	<li><a href="javascript:void(0);" onclick="thebuggenie.events.showFadedBackdrop('<?php echo make_url('get_partial_for_backdrop', array('key' => 'markasduplicate_issue', 'issue_id' => $issue->getID())); ?>');"><?php echo image_tag('icon_duplicate_issues.png', array('style' => 'float: left; margin-right: 5px;')); ?><?php echo __('Change the issue this is a duplicate of'); ?></a></li>
