<li class="hover_highlight<?php if ($issue->isClosed()): ?> closed<?php endif; ?>" id="related_issue_<?php echo $issue->getID(); ?>" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
	...
	<div style="float: left; display: inline-block; border: 1px solid rgba(0, 0, 0, 0.3); background-color: <?php echo ($issue->getStatus() instanceof TBGDatatype) ? $issue->getStatus()->getColor() : '#FFF'; ?>; font-size: 1px; width: 13px; height: 13px; margin: 2px 5px 0 2px;" title="<?php echo ($issue->getStatus() instanceof TBGDatatype) ? __($issue->getStatus()->getName()) : __('Status not determined'); ?>">&nbsp;</div>
