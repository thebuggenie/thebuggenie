<div class="rounded_box white<?php if (isset($absolute) && $absolute): ?> shadowed<?php endif; ?><?php if (isset($classes)): echo ' '.$classes; endif; ?>" id="<?php echo $html_id; ?>" style="<?php if (isset($absolute) && $absolute): ?>position: absolute;<?php else: ?>margin: 5px 0 5px 0; clear: both;<?php endif; ?> display: none; width: 324px; z-index: 10001; <?php if (isset($style)): foreach ($style as $key => $val): echo ' ' . $key . ': ' . $val . ';'; endforeach; endif; ?>">
	...
	<div class="dropdown_content">
		...
		<a href="javascript:void(0);" onclick="<?php echo str_replace(array(urlencode('%identifiable_type%'), '%identifiable_type%', urlencode('%identifiable_value%'), '%identifiable_value%'), array(TBGIdentifiableTypeClass::TYPE_USER, TBGIdentifiableTypeClass::TYPE_USER, $tbg_user->getID(), $tbg_user->getID()), $callback); ?>"><?php echo __('Select yourself'); ?> (<?php echo $tbg_user->getUsername(); ?>)</a><br>
		...
		<b><?php echo __('Or select one of your teams'); ?>:</b><br>
			<?php foreach ($tbg_user->getTeams() as $team): ?>
				<a href="javascript:void(0);" onclick="<?php echo str_replace(array(urlencode('%identifiable_type%'), '%identifiable_type%', urlencode('%identifiable_value%'), '%identifiable_value%'), array(TBGIdentifiableTypeClass::TYPE_TEAM, TBGIdentifiableTypeClass::TYPE_TEAM, $team->getID(), $team->getID()), $callback); ?>"><?php echo __('Select %teamname%', array('%teamname%' => $team->getName())); ?> (<?php echo $team->getName(); ?>)</a><br>
			<?php endforeach; ?>
		<?php endif; ?>
	</div>
