	<tr id="affected_<?php echo $itemtype; ?>_<?php echo $item['a_id']; ?>">
		...
		if ($issue->canEditIssue()): $url = "<a href=\"javascript:void(0);\" onClick=\"toggleConfirmed('".make_url('confirm_affected', array('issue_id' => $issue->getID(), 'affected_type' => $itemtype, 'affected_id' => $item['a_id']))."', '".$itemtype.'_'.$item['a_id']."');\">".$image."</a>"; else: $url = $image; endif; echo $url; ?><span id="affected_<?php echo $itemtype; ?>_<?php echo $item['a_id']; ?>_confirmed_spinner" style="display: none;"> <?php echo image_tag('spinning_16.gif'); ?></span></td>
	</tr>
