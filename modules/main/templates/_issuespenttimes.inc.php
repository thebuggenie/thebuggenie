			...
			<tbody>
				<?php foreach ($issue->getSpentTimes() as $spent_time): ?>
					<tr id="issue_spenttime_<?php echo $spent_time->getID(); ?>">
						...
						<td style="text-align: right;"><?php echo TBGIssue::getFormattedTime($spent_time->getSpentTime()); ?></td>
					...
					<?php if ($spent_time->getComment()): ?>
						<tr id="issue_spenttime_<?php echo $spent_time->getID(); ?>_comment">
							...
							<td colspan="4" class="faded_out" style="font-size: 0.9em; font-style: italic;">
								<?php echo htmlentities($spent_time->getComment(), ENT_COMPAT, TBGContext::getI18n()->getCharset()); ?>
							</td>
						</tr>
					<?php endif; ?>
				<?php endforeach; ?>
