	...
	class mainActionComponents extends BUGSactioncomponent
	{
		...
		public function componentIssuedetailslistEditable()
		{
			...
			foreach (BUGScustomdatatype::getAll() as $key => $customdatatype)
			{
				...
				$customfields_list[$key] = array('title' => $i18n->__($customdatatype->getDescription()), 'visible' => $customdatatype->isVisibleForIssuetype($this->issue->getIssuetype()->getID()), 'changed' => $this->issue->$changed_methodname(), 'merged' => $this->issue->$merged_methodname(), 'name' => (($this->issue->getCustomField($key) instanceof BUGScustomdatatypeoption) ? $this->issue->getCustomField($key)->getName() : ''), 'name_visible' => (bool) ($this->issue->getCustomField($key) instanceof BUGScustomdatatypeoption), 'noname_visible' => (bool) (!$this->issue->getCustomField($key) instanceof BUGScustomdatatypeoption), 'change_tip' => $i18n->__($customdatatype->getInstructions()), 'change_header' => $i18n->__($customdatatype->getDescription()), 'choices' => $customdatatype->getOptions(), 'clear' => $i18n->__('Clear this field'), 'select' => $i18n->__('%clear_this_field% or click to set a new value', array('%clear_this_field%' => '')));
			}
