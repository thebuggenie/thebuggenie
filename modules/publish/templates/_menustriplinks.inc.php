<li class="with-dropdown <?php if (strpos($selected_tab, 'publish_') === 0): ?>selected<?php endif; ?>">
    ...
    <?php if (!isset($wiki_url)): ?>
        <?= link_tag(((isset($project_url)) ? $project_url : $url), fa_image_tag('newspaper-o', array(), false, 'publish') . \thebuggenie\core\framework\Context::getModule('publish')->getMenuTitle($project instanceof \thebuggenie\core\entities\Project) . fa_image_tag('caret-down', ['class' => 'dropdown-indicator']), ['class' => $dropper_class]); ?>
    <?php else: ?>
    ...
    <div id="wiki_dropdown_menu" class="tab_menu_dropdown popup_box two-columns">
        <ul>
            ...
            <?php if ($project instanceof \thebuggenie\core\entities\Project): ?>
                <li><?php echo link_tag($wiki_url, \thebuggenie\core\framework\Context::getModule('publish')->getMenuTitle($project instanceof \thebuggenie\core\entities\Project), array('target' => 'blank')) ?></li>
            <?php endif; ?>
            <?php if (count(\thebuggenie\core\entities\Project::getAllRootProjects(false)) > (int) ($project instanceof \thebuggenie\core\entities\Project)): ?>
                ...
                <?php foreach (\thebuggenie\core\entities\Project::getAllRootProjects(false) as $root_project): ?>
                    ...
                    <?php if (!$root_project->hasWikiURL()): ?>
                        <li><?= link_tag(make_url('publish_article', array('article_name' => $root_project->getName().':MainPage')), image_tag($root_project->getSmallIconName(), array('class' => 'icon'), $root_project->hasSmallIcon()) . $root_project->getName()); ?></li>
                    <?php else: ?>
                        <li><?= link_tag($root_project->getWikiURL(), image_tag($root_project->getSmallIconName(), array('class' => 'icon'), $root_project->hasSmallIcon()) . $root_project->getName(), array('target' => 'blank')) ?></li>
                    <?php endif; ?>
                <?php endforeach; ?>
            <?php endif; ?>
        </ul>
