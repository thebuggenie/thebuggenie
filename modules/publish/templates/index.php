			...
			<ul id="article_list">
				...
				foreach (TBGContext::getModule('publish')->getAllArticles() as $article)
				{
					...
					<tr>
						...
						<td style="border-bottom: 1px solid #DDD; padding: 3px; width: 110px;"><?php echo tbg_formatTime($article->getPostedDate(), 3); ?></td>
					</tr>
					</table>
					</li>
					<?php 
		...
		else
		{
			...
			if ($featured_article != '')
			{
				...
				if ($f_article->isPublished())
				{
					...
					<div style="color: #AAA;"><b><?php print tbg_formatTime($f_article->getPostedDate(), 3); ?></b> by <b><?php echo $f_article->getAuthor(); ?></b></div>
					<div style="padding-top: 5px; font-size: 11.5px; padding-bottom: 5px; font-weight: bold;"><?php echo tbg_BBDecode($f_article->getIntro()); ?></div>
					...
					if (count($previous_articles) > 0)
					{
						...
						{ 
							...
							if ($article->getID() != $featured_article)
							{
								...
								<div style="color: #AAA;"><?php print tbg_formatTime($article->getPostedDate(), 3); ?> by <?php echo $article->getAuthor(); ?></div>
								<div style="padding-top: 5px; font-size: 11px; padding-bottom: 5px;"><?php echo tbg_BBDecode($article->getIntro()); ?></div>
