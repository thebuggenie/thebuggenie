<?php
    ...
    if (!TBGContext::isProjectContext() || (TBGContext::isProjectContext() && mb_strtolower($article_name) != mb_strtolower(TBGContext::getCurrentProject()->getKey() . ':mainpage')))
    {
        ...
        {
            $tbg_response->addBreadcrumb(TBGContext::getModule('publish')->getMenuTitle(), make_url('publish_article', array('article_name' => TBGContext::getCurrentProject()->getKey() . ':MainPage')), tbg_get_breadcrumblinks('project_summary', TBGContext::getCurrentProject()));
        }
        else
        {
            $tbg_response->addBreadcrumb(TBGContext::getModule('publish')->getMenuTitle(), make_url('publish_article', array('article_name' => 'MainPage')), tbg_get_breadcrumblinks('main_links'));
        }
