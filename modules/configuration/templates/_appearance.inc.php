	...
	<tr>
		...
		<td>
			<select name="icon_fav" id="icon_fav" onchange="if ($('icon_fav').getValue() == 2) { $('icon_fav_url').enable() } else { $('icon_fav_url').disable() }" style="width: 300px;"<?php if ($access_level != TBGSettings::ACCESS_FULL): ?> disabled<?php endif; ?>>
				...
				<option value="1"<?php if (TBGSettings::isUsingCustomFavicon() == '1'): ?> selected<?php endif; ?><?php if ($access_level != TBGSettings::ACCESS_FULL): ?> disabled<?php endif; ?>><?php echo __("Use the file favicon.png in the '%thebuggenie%' directory", array('%thebuggenie%', THEBUGGENIE_PUBLIC_FOLDER_NAME)); ?></option>
