	...
	<?php if ($teams): ?>
		<div style="margin: 5px 0 0 10px;">
			<?php foreach ($teams as $team): ?>
				<form action="<?php echo make_url('configure_project_add_assignee', array('project_id' => $theProject->getID(), 'assignee_type' => 'team', 'assignee_id' => $team->getID())); ?>" onsubmit="thebuggenie.events.assignToProject('<?php echo make_url('configure_project_add_assignee', array('project_id' => $theProject->getID(), 'assignee_type' => 'team', 'assignee_id' => $team->getID())); ?>', 'assign_team');return false;" method="post" id="assign_team">
	...
	<?php if ($users): ?>
		<div style="margin: 5px 0 0 10px;">
			<?php foreach ($users as $user): ?>
				<form action="<?php echo make_url('configure_project_add_assignee', array('project_id' => $theProject->getID(), 'assignee_type' => 'user', 'assignee_id' => $user->getID())); ?>" onsubmit="thebuggenie.events.assignToProject('<?php echo make_url('configure_project_add_assignee', array('project_id' => $theProject->getID(), 'assignee_type' => 'user', 'assignee_id' => $user->getID())); ?>', 'assign_user');return false;" method="post" id="assign_user">
