...
<div style="clear: both; margin-top: 10px; margin-bottom: 10px; width: 790px; height: 30px;" class="tab_menu">
	<ul id="editions_menu">
		<li<?php if ($selected_section == 'general'): ?> class="selected"<?php endif; ?> id="tab_edition_settings"><a href="javascript:void(0);" onclick="TBG.Main.Helpers.tabSwitcher('edition_settings', 'editions_menu');"><?php echo image_tag('cfg_icon_editiondetails.png', array('style' => 'float: left;')).__('Details &amp; settings'); ?></a></li>
		<li<?php if ($selected_section == 'components'): ?> class="selected"<?php endif; ?> id="tab_edition_components"><a href="javascript:void(0);" onclick="TBG.Main.Helpers.tabSwitcher('edition_components', 'editions_menu');"><?php echo image_tag('cfg_icon_components.png', array('style' => 'float: left;')).__('Components'); ?></a></li>
		<li<?php if ($selected_section == 'releases'): ?> class="selected"<?php endif; ?> id="tab_edition_builds"><a href="javascript:void(0);" onclick="TBG.Main.Helpers.tabSwitcher('edition_builds', 'editions_menu');"><?php echo image_tag('cfg_icon_builds.png', array('style' => 'float: left;')).__('Releases'); ?></a></li>
	</ul>
</div>
<div id="editions_menu_panes">
			...
			<?php if ($access_level == TBGSettings::ACCESS_FULL): ?>
				<td style="width: 380px; vertical-align: top; padding-left: 10px;">
					...
					<?php foreach ($edition->getProject()->getComponents() as $aComponent): ?>
						...
						<?php if ($access_level == TBGSettings::ACCESS_FULL): ?>
							<td style="width: 50px; text-align: left;"><a href="javascript:void(0);" onclick="TBG.Project.Edition.addComponent('<?php echo make_url('configure_edition_add_component', array('project_id' => $edition->getProject()->getID(), 'edition_id' => $edition->getID(), 'component_id' => $aComponent->getID())); ?>', <?php echo $aComponent->getID(); ?>);">&lt;&lt;&nbsp;<?php echo __('Add'); ?></a></td>
						<?php endif; ?>
							<td style="width: 20px; padding: 2px;"><?php echo image_tag('icon_components.png'); ?></td>
							<td style="width: auto; padding: 2px;"><?php print $aComponent; ?></td>
						</tr>
	...
	<div id="edition_builds_pane" style="width: 790px; text-align: left;<?php if ($selected_section != 'releases'): ?> display: none;<?php endif; ?>">
