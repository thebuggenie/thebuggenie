<?php if (count($permissions_list) > 0): ?>
	<?php foreach ($permissions_list as $permission_key => $permission): ?>
		<?php if (is_numeric($permission_key)): ?>
			<?php include_template('configuration/permissionsblock', array('base_id' => $base_id.$permission_key, 'permissions_list' => $permission, 'mode' => $mode, 'target_id' => $target_id, 'module' => $module, 'access_level' => $access_level)); ?>
		<?php else: ?>
			...
			<li>
				...
				<?php if(array_key_exists('details', $permission) && count($permission['details']) > 0): ?>
					<a href="javascript:void(0);" onclick="<?php if (!array_key_exists('iscontainer', $permission) || $permission['iscontainer'] == false): ?>$('<?php echo $base_id; ?>_<?php echo $permission_key; ?>_settings').hide();<?php endif; ?>getPermissionOptions('<?php echo make_url('configure_permissions_get_permissions', array('base_id' => $base_id . $permission_key, 'permissions_list' => $permission_key, 'mode' => $mode, 'target_id' => $current_target_id, 'target_module' => $module)); ?>', '<?php echo $base_id; ?>_<?php echo $permission_key; ?>_details');" style="float: right; margin-right: 5px;" title="<?php echo __('More fine-tuned permissions are available. Click to see them.'); ?>"><?php echo image_tag('icon_project_permissions.png'); ?></a>
				<?php endif; ?>
