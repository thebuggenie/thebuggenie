			...
			<li>
				<?php if (!isset($user_id) || !$user_id): ?>
					<?php if (!array_key_exists('iscontainer', $permission) || $permission['iscontainer'] == false): ?>
						<a href="javascript:void(0);" onclick="<?php if(array_key_exists('details', $permission) && count($permission['details']) > 0): ?>$('<?php echo $base_id; ?>_<?php echo $permission_key; ?>_details').hide();<?php endif; ?>TBG.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'permissions', 'permission_key' => $permission_key, 'item_name' => $permission['description'], 'mode' => $mode, 'target_id' => $current_target_id, 'target_module' => $module, 'access_level' => $access_level)); ?>');" style="float: right;"><?php echo image_tag('cfg_icon_permissions.png'); ?></a>
					<?php endif; ?>
				<?php else: ?>
				...
				<?php endif; ?>
				<?php if (!isset($user_id) || !$user_id): ?>
					<a href="javascript:void(0);" onclick="<?php if(array_key_exists('details', $permission) && count($permission['details']) > 0): ?>$('<?php echo $base_id; ?>_<?php echo $permission_key; ?>_details').hide();<?php endif; ?>TBG.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'permissions', 'permission_key' => $permission_key, 'item_name' => $permission['description'], 'mode' => $mode, 'target_id' => $current_target_id, 'target_module' => $module, 'access_level' => $access_level)); ?>');" class="permission_description"><?php echo $permission['description']; ?></a>
				<?php else: ?>
