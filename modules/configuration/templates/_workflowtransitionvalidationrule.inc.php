			...
			case TBGWorkflowTransitionValidationRule::RULE_MAX_ASSIGNED_ISSUES:
				...
				<?php if (!$rule->getTransition()->isCore()): ?>
					<td id="workflowtransitionvalidationrule_<?php echo $rule->getID(); ?>_edit" style="display: none; padding: 2px;">
						<form action="<?php echo make_url('configure_workflow_transition_update_validation_rule', array('workflow_id' => $rule->getWorkflow()->getID(), 'transition_id' => $rule->getTransition()->getID(), 'rule_id' => $rule->getID())); ?>" onsubmit="thebuggenie.events.updateWorkflowTransitionValidationRule('<?php echo make_url('configure_workflow_transition_update_validation_rule', array('workflow_id' => $rule->getWorkflow()->getID(), 'transition_id' => $rule->getTransition()->getID(), 'rule_id' => $rule->getID())); ?>', <?php echo $rule->getID(); ?>);return false;" id="workflowtransitionvalidationrule_<?php echo $rule->getID(); ?>_form">
		...
		<div class="rounded_box white shadowed" style="position: absolute; width: 285px; display: none;" id="workflowtransitionvalidationrule_<?php echo $rule->getID(); ?>_delete">
			...
			<div class="content">
				...
				<div style="text-align: right;">
					<?php echo javascript_link_tag(__('Yes'), array('onclick' => "thebuggenie.events.deleteWorkflowTransitionValidationRule('".make_url('configure_workflow_transition_delete_validation_rule', array('workflow_id' => $rule->getWorkflow()->getID(), 'transition_id' => $rule->getTransition()->getID(), 'rule_id' => $rule->getID()))."', {$rule->getID()}, '{$rule->isPreOrPost()}', '{$rule->getRule()}');")); ?> ::
