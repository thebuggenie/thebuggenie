	...
	<div style="width: 750px;" id="config_issuefields">
		...
		<?php foreach ($builtin_types as $type_key => $type): ?>
			<div class="rounded_box borderless mediumgrey" style="margin: 5px 0 0 0; font-size: 12px;">
				...
				<div class="header"><a href="javascript:void(0);" onclick="thebuggenie.events.showIssuefieldOptions('<?php echo make_url('configure_issuefields_getoptions', array('type' => $type_key)); ?>', '<?php echo $type_key; ?>');"><?php echo $type['description']; ?></a>&nbsp;<span class="faded_out dark" style="font-weight: normal; font-size: 12px;"><?php echo $type['key']; ?></span></div>
		...
		<div class="rounded_box yellow borderless" style="margin: 5px 0 0 0; font-size: 12px;">
			...
			<form accept-charset="<?php echo TBGContext::getI18n()->getCharset(); ?>" action="<?php echo make_url('configure_issuefields_add_customtype'); ?>" onsubmit="thebuggenie.events.addIssuefieldCustom('<?php echo make_url('configure_issuefields_add_customtype'); ?>');return false;" id="add_custom_type_form">
