		...
		<div id="user_<?php echo $user->getID(); ?>_delete" style="position: absolute; left: 280px; width: 700px; z-index: 100; display: none;" class="rounded_box white shadowed">
			...
			<div class="content">
				...
				<div style="text-align: right;">
					<?php echo javascript_link_tag(__('Yes'), array('onclick' => "thebuggenie.events.deleteUser('".make_url('configure_users_delete_user', array('user_id' => $user->getID()))."', ".$user->getID().");")); ?>
					 ::
...
<td style="padding: 3px; position: relative;">
	<?php echo javascript_link_tag(image_tag('icon_edit.png', array('title' => __('Edit this user'))), array('onclick' => "thebuggenie.events.loadUserEditForm('".make_url('configure_users_edit_user_form', array('user_id' => $user->getID()))."', ".$user->getID().");$('users_results_user_".$user->getID()."').toggle();", 'class' => 'image')); ?>
	<?php echo javascript_link_tag(image_tag('cfg_icon_permissions.png', array('title' => __('Edit permissions for this user'))), array('onclick' => "thebuggenie.events.getUserPermissionsBlock('".make_url('configure_permissions_get_configurator', array('user_id' => $user->getID(), 'base_id' => $user->getID())). "', ".$user->getID().");", 'class' => 'image', 'id' => 'permissions_'.$user->getID().'_link')); ?>
