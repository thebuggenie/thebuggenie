...
<tr id="edition_box_<?php echo $anEdition->getID(); ?>" class="canhover_light">
	<td style="width: 20px; padding: 2px;"><?php echo image_tag('icon_edition.png'); ?></td>
	<td style="width: auto; padding: 2px;"><?php echo link_tag(make_url('configure_project_edition', array('project_id' => $anEdition->getProject()->getID(), 'edition_id' => $anEdition->getID(), 'mode' => 'general')), '<b>'.$anEdition->getName().'</b>'); ?><?php if ($anEdition->isDefault()): echo __('%edition_name% (default)', array('%edition_name%' => '')); endif; ?></td>
...
<tr>
	...
	<?php if ($anEdition->hasDescription()): ?>
		<div style="padding-bottom: 10px; color: #AAA;"><?php print $anEdition->getDescription(); ?></div>
	<?php endif; ?>
	<?php if (BUGScontext::getRequest()->isAjaxCall()): ?>
		<script type="text/javascript">new Effect.Pulsate('edition_box_<?php echo $anEdition->getID(); ?>');</script>
	<?php endif; ?>	
	</td>
</tr>