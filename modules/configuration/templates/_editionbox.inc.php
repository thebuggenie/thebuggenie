...
<tr id="edition_box_<?php echo $edition->getID(); ?>" class="canhover_light">
	<td style="width: auto; padding: 2px;"><?php echo image_tag('icon_edition.png', array('style' => 'float: left; margin-right: 5px;')); ?> <?php echo $edition->getName(); ?><?php if ($edition->isDefault()): echo __('%edition_name% (default)', array('%edition_name%' => '')); endif; ?></td>
	<td style="width: 40px; padding: 2px;">
		<?php echo javascript_link_tag(image_tag('icon_edit.png'), array('onclick' => "editEdition('".make_url('configure_project_edition', array('project_id' => $edition->getProject()->getID(), 'edition_id' => $edition->getID()))."', '".$edition->getID()."');", 'title' => __('Edit edition'))); ?>
		<a class="image" href="javascript:void(0);" onclick="Effect.Appear('del_edition_<?php echo $edition->getID(); ?>', { duration: 0.5 });"><?php echo image_tag('action_cancel_small.png'); ?></a><br>
