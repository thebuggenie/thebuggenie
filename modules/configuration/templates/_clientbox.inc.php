<div class="rounded_box mediumgrey borderless" style="padding: 0; margin-top: 5px;" id="clientbox_<?php echo $client->getID(); ?>">
	<div style="padding: 5px;">
		...
		<?php echo javascript_link_tag(image_tag('client_list_users.png'), array('title' => __('List users in this client'), 'onclick' => 'showTeamMembers(\''.make_url('configure_users_get_client_members', array('client_id' => $client->getID())).'\', '.$client->getID().');', 'style' => 'float: right; margin-right: 5px;', 'class' => 'image')); ?>
		...
		<div class="rounded_box white shadowed" style="margin: 5px; display: none;" id="confirm_client_<?php echo $client->getID(); ?>_delete">
			...
			<div class="dropdown_content">
				...
				<div style="text-align: right;">
					<?php echo javascript_link_tag(__('Yes'), array('onclick' => 'deleteTeam(\''.make_url('configure_users_delete_client', array('client_id' => $client->getID())).'\', '.$client->getID().');')); ?> :: <b><?php echo javascript_link_tag(__('No'), array('onclick' => '$(\'confirm_client_'.$client->getID().'_delete\').toggle();')); ?></b>
				</div>
