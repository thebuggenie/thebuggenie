<?php if (in_array($tbg_response->getPage(), array('agile_board', 'agile_whiteboard'))): ?>
    ...
    <div class="project_header_right button-group inset">
        ...
        <?php if ($tbg_response->getPage() == 'agile_board'): ?>
            <?php if ($tbg_user->canManageProject(\thebuggenie\core\framework\Context::getCurrentProject())): ?>
                <a href="javascript:void(0);" class="planning_board_settings_gear" onclick="TBG.Main.Helpers.Backdrop.show('<?php echo make_url('get_partial_for_backdrop', array('key' => 'agileboard', 'project_id' => $board->getProject()->getID(), 'board_id' => $board->getID())); ?>');" title="<?php echo __('Edit this board'); ?>"><?php echo fa_image_tag('cog'); ?></a>
            <?php endif; ?>
        <?php endif; ?>
    </div>
<?php endif; ?>
