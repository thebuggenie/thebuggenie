		...
		<?php elseif (isset($permissions_ok) && $permissions_ok): ?>
			...
			<form accept-charset="utf-8" action="<?php echo make_url('upgrade'); ?>" method="post" onsubmit="if (!$('confirm_backup').checked) { return false; } else { $('upgrading_popup').show(); }">
				...
				<div class="padded_box installpage backup" id="install_page_2">
					...
					<div class="progress_buttons">
						<button class="button button-silver button-previous" onclick="tbg_upgrade_previous($(this).up('.installpage'));">Previous</button>
						<button class="button button-silver button-next" onclick="tbg_upgrade_next($(this).up('.installpage'));">Next</button>
					</div>
				</div>
				...
				<div class="padded_box installpage" id="install_page_4">
					...
					<ul class="passwordlist">
						<li class="greybox">
							...
							<input type="text" name="manul_password" disabled placeholder="Specify a password for all users here" id="upgrade_password_manual_input" onkeyup="if ($(this).getValue().length >= 5) { $('upgrade_password_continue').enable(); } else { $('upgrade_password_continue').disable(); }">&nbsp;Must be at least 5 characters
						</li>
						<li class="greybox">
					...
					<div class="progress_buttons">
						<button class="button button-silver button-previous" onclick="tbg_upgrade_previous($(this).up('.installpage'));">Previous</button>
						<button class="button button-silver button-next" id="upgrade_password_continue" disabled onclick="tbg_upgrade_next($(this).up('.installpage'));">Next</button>
					</div>
				</div>
