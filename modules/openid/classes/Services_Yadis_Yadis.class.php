 ...
 *  $fetcher = self::getHTTPFetcher();
 *  $yadis_object = self::discover($uri,
 *                                    $http_response, $fetcher);</pre>
 *
 * If the discovery succeeds, $yadis_object will be an instance of
 * {@link self}.  If not, it will be null.  The XRDS
 ...
 *                          openidActions::SERVICES_YADIS_MATCH_ALL);</pre>
 *
 * See {@link openidActions::SERVICES_YADIS_MATCH_ALL} and {@link
 * openidActions::SERVICES_YADIS_MATCH_ANY}.
 *
...
class Services_Yadis_Yadis
{
     ...
     * @access private
     */
    protected static function _getHeader($header_list, $names)
    {
     ...
     * @access private
     */
    protected static function _getContentType($content_type_header)
    {
     ...
     * self, depending on whether the discovery
     * succeeded.
     */
    public static function discover($uri, &$http_response, $fetcher, $extra_ns_map = null, $timeout = 20)
    {
        if (!$uri)
		{
            return null;
        ...
        if (!$fetcher) {
            $fetcher = self::getHTTPFetcher($timeout);
        }
        ...
        $xrds_header_uri = self::_getHeader($response->headers, array('x-xrds-location', 'x-yadis-location'));

		//var_dump($xrds_header_uri);die();
        $content_type = self::_getHeader($response->headers, array('content-type'));
        ...
        if ($xrds_header_uri) {
			...
			else
			{
                ...
                $content_type = self::_getHeader($headers, array('content-type'));
            }
        }

        if (self::_getContentType($content_type) != 'application/xrds+xml')
		{
    ...
    /**
     * Instantiates an empty self object.  This
