<?php
    ...
    use thebuggenie\modules\publish\entities\Article,
        ...
        thebuggenie\modules\mailing\entities\b2db\MailQueueTable,
        Swift_Message,
        Swift_Mime_Message,
        Swift_Mailer,
        Swift_MailTransport,
    ...
    class Mailing extends \TBGModule
    {
        ...
        protected function _addListeners()
        {
            ...
            TBGEvent::listen('core', '\thebuggenie\modules\publish\entities\Article::doSave', array($this, 'listen_Article_doSave'));
            ...
            TBGEvent::listen('core', 'config_project_tabs_settings', array($this, 'listen_projectconfig_tab_settings'));
            TBGEvent::listen('core', 'config_project_tabs_other', array($this, 'listen_projectconfig_tab_other'));
...
EOT;
        ...
        public function getUsersAndLanguages($users)
        {
            ...
            {
                if (is_numeric($user))
                    $user = TBGContext::factory()->TBGUser($user);
...
//                __('Forgot your password?');
        ...
        protected function _getArticleRelatedUsers(Article $article, TBGUser $triggered_by_user)
        {
        ...
        public function listen_issueCreate(TBGEvent $event)
        {
            if ($this->isOutgoingNotificationsEnabled())
            {
                ...
                if ($issue instanceof TBGIssue)
                {
                    ...
                    if (!$this->getSetting(self::NOTIFY_UPDATED_SELF, TBGContext::getUser()->getID()))
                        unset($to_users[TBGContext::getUser()->getID()]);
        ...
        {
            if (!$event->getSubject() instanceof TBGIssue)
                return;
