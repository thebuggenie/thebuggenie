        ...
        public function runConfigureProjectSettings(TBGRequest $request)
        {
            ...
            if ($this->access_level != TBGSettings::ACCESS_FULL)
            {
                ...
                if ($request['mailing_from_address'] != '')
                {
                    ...
                    {
                        TBGContext::getModule('mailing')->saveSetting(Mailing::SETTING_PROJECT_FROM_ADDRESS . $project_id, trim(mb_strtolower($request->getParameter('mailing_from_address'))));
                        if (trim($request['mailing_from_name']) !== '')
                        {
                            TBGContext::getModule('mailing')->saveSetting(Mailing::SETTING_PROJECT_FROM_NAME . $project_id, trim($request->getParameter('mailing_from_name')));
                        }
                        else
                        {
                            TBGContext::getModule('mailing')->deleteSetting(Mailing::SETTING_PROJECT_FROM_NAME . $project_id);
                        }
                    }
                    else
                    {
                ...
                {
                    TBGContext::getModule('mailing')->deleteSetting(Mailing::SETTING_PROJECT_FROM_ADDRESS . $project_id);
                    TBGContext::getModule('mailing')->deleteSetting(Mailing::SETTING_PROJECT_FROM_NAME . $project_id);
                }
