<?php if ($issue instanceof TBGIssue): ?>
	<h3>
		...
		<span style="font-size: 0.8em; font-weight: normal;"><?php echo __('Updated by') . ' ' . $updated_by->getBuddyname() . ' (' . $updated_by->getUsername() . ')'; ?></span><br>
		<span style="font-size: 0.8em; color: #AAA; font-weight: normal;"><?php echo __('Created by') . ' ' . $issue->getPostedBy()->getBuddyname() . ' (' . $issue->getPostedBy()->getUsername() . ')'; ?></span>
	</h3>
	<?php if (isset($comment) && $comment instanceof TBGComment): ?>
		<h4><?php echo __('Comment by') . ' ' . $comment->getPostedBy()->getBuddyname() . ' (' . $comment->getPostedBy()->getUsername() . ')'; ?></h4>
	...
	<div style="color: #888;">
		...
		<?php if (isset($comment) && $comment instanceof TBGComment){echo __('Show comment:') . ' ' . link_tag($module->generateURL('viewissue', array('project_key' => $issue->getProject()->getKey(), 'issue_no' => $issue->getFormattedIssueNo())).'#comment_'.$comment->getID()) . "<br>";} ?>
		<?php echo __('Show') . ' ' . $issue->getProject()->getName() . ' ' . __('project dashboard:') . ' ' . link_tag($module->generateURL('project_dashboard', array('project_key' => $issue->getProject()->getKey()))); ?><br>
		<br>
