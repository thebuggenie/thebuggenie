		...
		switch ($filter->getFilterKey())
		{
			case 'project_id':
				...
				<?php if (TBGContext::isProjectContext()): ?>
					...
					<input type="hidden" name="filters[project_id][value]" value="<?php echo TBGContext::getCurrentProject()->getID(); ?>" id="filter_project_id_value_input">
				<?php else: ?>
			...
			case 'assignee_user':
				...
				<div class="filter interactive_dropdown" id="interactive_filter_<?php echo $filter->getFilterKey(); ?>" data-filterkey="<?php echo $filter->getFilterKey(); ?>" data-value="<?php echo $filter->getValue(); ?>" data-all-value="<?php echo __('Anyone'); ?>">
					<input type="hidden" name="filters[<?php echo $filter->getFilterKey(); ?>][operator]" value="=">
					<input type="hidden" name="filters[<?php echo $filter->getFilterKey(); ?>][value]" value="" id="filter_<?php echo $filter->getFilterKey(); ?>_value_input">
					<label><?php 
			...
			case 'last_updated':
				...
				?>
				<div class="filter interactive_dropdown" id="interactive_filter_<?php echo $key; ?>" data-filterkey="<?php echo $key; ?>" data-value="<?php echo $filter->getValue(); ?>" data-all-value="<?php echo __('Today'); ?>" data-isdate>
					<input type="hidden" name="filters[<?php echo $key; ?>][operator]" value="=" id="filter_<?php echo $key; ?>_operator_input">
					<input type="hidden" name="filters[<?php echo $key; ?>][value]" value="<?php echo (int) $filter->getValue(); ?>" data-display-value="<?php echo (date('Ymd', (int) $filter->getValue()) == date('Ymd')) ? __('Today') : date('Y-m-d', (int) $filter->getValue()); ?>" id="filter_<?php echo $key; ?>_value_input">
					<label><?php echo $title; ?></label>
					<div class="interactive_menu">
						<h1><?php echo $description; ?></h1>
						<div class="interactive_values_container">
							<ul class="interactive_menu_values">
								<li data-value="&lt;=" class="filtervalue<?php if ($filter->getOperator() == '<=') echo ' selected'; ?>" data-operator data-exclusive data-selection-group="1">
									<?php echo image_tag('icon-mono-checked.png', array('class' => 'checked')); ?>
									<input type="checkbox" value="&lt;=" name="filters_<?php echo $key; ?>_operator_before" data-text="<?php echo __('Before %time', array('%time' => '')); ?>" id="filters_<?php echo $key; ?>_operator_before" <?php if ($filter->getOperator() == '<=') echo 'checked'; ?>>
									<label for="filters_<?php echo $key; ?>_value_before"><?php echo __('Before %time', array('%time' => '')); ?></label>
								</li>
								<li data-value="&gt;=" class="filtervalue<?php if ($filter->getOperator() == '>=') echo ' selected'; ?>" data-operator data-exclusive data-selection-group="1">
									<?php echo image_tag('icon-mono-checked.png', array('class' => 'checked')); ?>
									<input type="checkbox" value="&gt;=" name="filters_<?php echo $key; ?>_operator_after" data-text="<?php echo __('After %time', array('%time' => '')); ?>" id="filters_<?php echo $key; ?>_operator_after" <?php if ($filter->getOperator() == '>=') echo 'checked'; ?>>
									<label for="filters_<?php echo $key; ?>_value_after"><?php echo __('After %time', array('%time' => '')); ?></label>
								</li>
								<li class="separator"></li>
							</ul>
						</div>
