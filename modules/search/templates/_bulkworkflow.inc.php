		...
		<?php else: ?>
			...
			<div class="content">
				<?php foreach ($available_transitions as $transition): ?>
					<div><?php echo javascript_link_tag($transition->getName(), array('onclick' => "TBG.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'workflow_transition', 'transition_id' => $transition->getID()))."&issue_ids[]=".join('&issue_ids[]=', array_keys($issues))."');", 'class' => 'button button-silver')); ?></div>
				<?php endforeach; ?>
			</div>
		<?php endif; ?>
	</div>
