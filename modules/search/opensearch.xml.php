<?php
	...
	if (BUGScontext::getModule('search')->getSetting('enable_opensearch') != 1)
	{
		exit();
	}
...
<ShortName><?php print BUGScontext::getModule('search')->getSetting('opensearch_title'); ?></ShortName>
<LongName><?php print BUGScontext::getModule('search')->getSetting('opensearch_longname'); ?></LongName>
<Description><?php print BUGScontext::getModule('search')->getSetting('opensearch_description'); ?></Description>
...
<Url type="text/html" template="<?php echo BUGSsettings::get('url_host') . BUGSsettings::get('url_subdir'); ?>modules/search/search.php?scope=<?php echo BUGScontext::getScope()->getID(); ?>&amp;searchfor={searchTerms}"/>
<Url type="application/x-suggestions+json" template="<?php echo BUGSsettings::get('url_host') . BUGSsettings::get('url_subdir'); ?>modules/search/search_stripped.php?scope=<?php echo BUGScontext::getScope()->getID(); ?>&amp;output=json&amp;searchfor={searchTerms}"/>
<AdultContent>false</AdultContent>
<OutputEncoding>UTF-8</OutputEncoding>
<Contact><?php print BUGScontext::getModule('search')->getSetting('opensearch_contact'); ?></Contact>
