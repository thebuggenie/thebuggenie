<?php
	...
	require TBGContext::getIncludePath() . 'modules/messages/newmessage_logic.inc.php';
	require TBGContext::getIncludePath() . "include/header.inc.php";
	require TBGContext::getIncludePath() . "include/menustrip.inc.php";
...
<?php
	...
	if (TBGContext::getRequest()->getParameter('message_findsendto'))
	{
		?>
		...
		<?php
		$matchUsers = array();
		$matchUsers = TBGUser::getUsers(TBGContext::getRequest()->getParameter('message_findsendto'), false, true);
		if ($matchUsers != false)
		{
			foreach ($matchUsers as $aUser)
			{
				$aUser = new TBGUser($aUser['id']);
				?>
	...
	{
		?>
		<form accept-charset="<?php echo TBGContext::getI18n()->getCharset(); ?>" action="newmessage.php" enctype="multipart/form-data" method="post" name="message_setsendto">
		<input type="hidden" name="clear_sendto" value="true">
		<table cellpadding=0 cellspacing=0 style="width: 100%;">
		<tr>
			<td style="width: auto; padding: 2px;"><input type="text" style="width: 100%;" name="message_findsendto" value="<?php print TBGContext::getRequest()->getParameter('message_findsendto', ''); ?>"></td>
	...
	else
	{
		...
		<form accept-charset="<?php echo TBGContext::getI18n()->getCharset(); ?>" action="newmessage.php" enctype="multipart/form-data" method="post" name="message_sendto_done">
		<input type="hidden" name="dosendmessage" value="true">
		<tr>
		<td style="background-color: #F1F1F1; padding: 5px;"><b><?php echo __('Title'); ?></b></td>
		<td style="background-color: #F1F1F1; padding: 5px;" colspan=2><input type="text" name="message_title" value="<?php print TBGContext::getRequest()->getParameter('set_title', ''); ?>" style="width: 100%;"></td>
		</tr>
		<tr>
