...
<?php
	...
	if (count($all_messages) > 0)
	{
		foreach ($all_messages as $aMessage)
		{
			...
			<div style="padding: 2px; border-bottom: 1px solid #F1F1F1;"><a href="messages.php?msg_id=<?php print $aMessage->get(B2tMessages::ID); ?>#msg<?php print $aMessage->get(B2tMessages::ID); ?>"><?php print ($aMessage->get(B2tMessages::IS_READ) == 0) ? "<b>" : ""; print bugs_BBDecode($aMessage->get(B2tMessages::TITLE)); print ($aMessage->get(B2tMessages::IS_READ) == 0) ? "</b>" : ""; ?></a>
			...
			?>, <?php print bugs_formatTime($aMessage->get(B2tMessages::SENT), 3); ?></div>
			</div>
			</td>
			</tr>
			<?php
	...
	if (is_array($the_msg) || $messages_viewmode == 2)
	{
		...
		<?php if (is_array($the_msg)): ?>
			...
			<?php
				...
				$replystring = str_replace("{uid}", $the_msg[0]->get(B2tMessages::FROM_USER) . "&amp;set_title=RE: " . bugs_BBDecode($the_msg[0]->get(B2tMessages::TITLE)), $replystring);
		...
		<?php if (is_array($the_msg)): ?>
			...
			<b><?php echo __('Sent:'); ?>&nbsp;</b><?php print bugs_formatTime($the_msg[0]->get(B2tMessages::SENT), 7); ?><br>
			<table cellpadding=0 cellspacing=0 style="width: 100%;">
			<tr>
			<td style="width: 35px;" valign="middle"><b><?php echo __('From:') ?></b></td>
			<td><table cellpadding=0 cellspacing=0 style="width: 100%;"><?php print bugs_userDropdown($the_msg[0]->get(B2tMessages::FROM_USER)); ?></table></td>
			</tr>
			</table>
			<div style="font-size: 13px; width: auto; padding: 3px; padding-top: 5px; padding-left: 0px;"><b><?php print bugs_BBDecode($the_msg[0]->get(B2tMessages::TITLE)); ?></b></div>
			</div>
			<div style="padding: 5px;">
			<?php
			print bugs_BBDecode($the_msg[0]->get(B2tMessages::BODY));
			?>
			</div>
		<?php endif; ?>
		<?php
